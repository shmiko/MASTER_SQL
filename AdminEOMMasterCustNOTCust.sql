...
var Notcust varchar2(20)  --Use this to exclude Parent when running split cost centres
exec :Notcust := 'TABCORP' -- For example when running AAS Non FUND - this would be set to AAS and the cust would be AAS NON FUND
...
exec :start_date := To_Date('29-May-2014')
...
exec :end_date := To_Date('31-May-2014')
  ...
  var nCountCostCenterStocks NUMBER /*VerbalOrderEntryFee*/
  exec SELECT  Count(IM_STOCK) INTO :nCountCostCenterStocks FROM IM where IM_XX_COST_CENTER01 = :cust AND IM_ACTIVE = 1;
...
/*freight fees*/
	 select    s.SH_CUST                AS "Customer",
        ...
        CASE  WHEN regexp_substr(s.SH_SPARE_STR_3,'[a-z]+', 1, 2) IS NOT NULL THEN  s.SH_SPARE_STR_3 || '@' || s.SH_SPARE_STR_1
           ...
           EOM_REPORT_PKG_TEST.f_GetWarehouse_from_SD(s.SL_LOCN) AS Warehouse
	FROM      PWIN175.SD d
	...
	AND       (r.RM_PARENT != :Notcust OR r.RM_CUST != :Notcust)
  ...
  HAVING EOM_REPORT_PKG_TEST.f_GetWarehouse_from_SD(s.SL_LOCN) LIKE :warehouse
	GROUP BY  s.SH_CUST,
			  s.SH_SPARE_STR_4,
			  s.SH_ORDER,
			  t.ST_PICK,
...
SELECT    s.SH_CUST                AS "Customer",
			  r.RM_PARENT              AS "Parent",
	   CASE   WHEN i.IM_CUST <> :cust2 AND s.SH_SPARE_STR_4 IS NULL THEN s.SH_CUST
            WHEN i.IM_CUST <> :cust2 THEN s.SH_SPARE_STR_4
	   ...
	   CASE   WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN d.SD_SELL_PRICE
            ...
            WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NOT NULL THEN eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK)
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NULL THEN  d.SD_XX_OW_UNIT_PRICE
			      ELSE NULL
			      END                        AS "Batch/UnitPrice",
	   CASE   WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 THEN To_Number(i.IM_REPORTING_PRICE)
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 THEN  eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK)
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NULL THEN  d.SD_XX_OW_UNIT_PRICE
			      ELSE NULL
			      END                        AS "OWUnitPrice",
      CASE  WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN d.SD_SELL_PRICE * l.SL_PSLIP_QTY
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 1 THEN (n.NI_SELL_VALUE/n.NI_NX_QUANTITY) * l.SL_PSLIP_QTY
            WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NOT NULL THEN  eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) * l.SL_PSLIP_QTY
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NULL THEN  d.SD_XX_OW_UNIT_PRICE * l.SL_PSLIP_QTY
			      ELSE NULL
			      END          AS "DExcl",
	   ...
	   CASE   WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 THEN To_Number(i.IM_REPORTING_PRICE)
			      ...
			      ELSE NULL
			      END                       AS "Excl_Total",
	  CASE    WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN (d.SD_SELL_PRICE * l.SL_PSLIP_QTY) * 1.1
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 1  THEN  ((n.NI_SELL_VALUE/n.NI_NX_QUANTITY) * l.SL_PSLIP_QTY) * 1.1
            WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NOT NULL THEN  (eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) * l.SL_PSLIP_QTY) * 1.1
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NULL THEN  (d.SD_XX_OW_UNIT_PRICE * l.SL_PSLIP_QTY) * 1.1
			      ELSE NULL
			      END          AS "DIncl",
	   CASE   WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN (d.SD_SELL_PRICE * l.SL_PSLIP_QTY) * 1.1
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 1 THEN  ((n.NI_SELL_VALUE/n.NI_NX_QUANTITY) * l.SL_PSLIP_QTY) * 1.1
...
/* EOM Storage Fees */
  ...
  INNER JOIN Tmp_Locn_Cnt_By_Cust ON sLocn = l1.IL_LOCN
	...
	GROUP BY IM_CUST,IM_XX_COST_CENTRE01,n1.NI_LOCN,n1.NI_AVAIL_ACTUAL,5,6,n1.NI_STOCK,8,9,10,11,12,l1.IL_NOTE_2,n1.NI_MADE_DATE,IM_LEVEL_UNIT,l1.IL_LOCN,nCountOfStocks,IM_BRAND,IM_OWNED_By,IM_PROFILE
