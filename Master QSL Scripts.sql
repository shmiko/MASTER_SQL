...
Select f1.DESPDATE,
            ...
            -- (Select Count(*) From TMP_ALL_FEES_F Where ((ADDRESS LIKE '%Casselden%' Or ADDRESS LIKE '%Lonsdale%') OR (ADDRESS2 LIKE '%Casselden%' Or ADDRESS2 LIKE '%Lonsdale%'))) THEN '30.71'
              CASE  WHEN F_DAILY_FREIGHT_COUNT(startdate,enddate) >= 1 AND to_char(date DESPDATE, 'D') = 1 THEN '30.71' --monday
                    WHEN F_DAILY_FREIGHT_COUNT(startdate,enddate) >= 1 AND to_char(date DESPDATE, 'D') = 2 THEN '30.71' --tuesday
                    WHEN F_DAILY_FREIGHT_COUNT(startdate,enddate) >= 1 AND to_char(date DESPDATE, 'D') = 3 THEN '30.71' --wednesday
                    WHEN F_DAILY_FREIGHT_COUNT(startdate,enddate) >= 1 AND to_char(date DESPDATE, 'D') = 4 THEN '30.71' --thursday
                    WHEN F_DAILY_FREIGHT_COUNT(startdate,enddate) >= 1 AND to_char(date DESPDATE, 'D') = 5 THEN '30.71' --friday
                    ELSE 0
 ...
 Select  f1.DESPDATE,f1.ORDERNUM,f1.DESPNOTE,f1.CUSTOMER,
        ...
        f1.ITEM,f1.DESCRIPTION,f1.QTY
            ...
            ,CASE   WHEN f1.FEETYPE like 'Stock' AND LAG(f1.DESPNOTE, 1, 0) OVER (ORDER BY f1.DESPNOTE) != f1.DESPNOTE THEN (Select f2.SELLEXCL From TMP_ALL_FEES_F f2 Where f2.ORDERNUM = f1.ORDERNUM AND f2.FEETYPE = 'Handeling Fee is ' ) --As "Line Charge"-- AND LAG(FEETYPE, 1, 0) OVER (ORDER BY FEETYPE) = 'Pick Fee'  THEN LEAD(SELLEXCL, 2, 0) OVER (ORDER BY SELLEXCL)
                    ELSE 0
                    END AS "Order Despatch Charge"
            ,CASE   WHEN f1.FEETYPE like 'Stock' AND (LAG(f1.DESPNOTE, 1, 0) OVER (ORDER BY f1.DESPNOTE) != f1.DESPNOTE)  AND ((UPPER(ADDRESS) NOT LIKE '%CASSELDEN%' Or UPPER(ADDRESS) NOT LIKE '2 LONSDALE%') OR (UPPER(ADDRESS2) NOT LIKE '%CASSELDEN%' Or UPPER(ADDRESS2) NOT LIKE '2 LONSDALE%'))
                    THEN (
                          Select f2.SELLEXCL From TMP_ALL_FEES_F f2 Where f2.ORDERNUM = f1.ORDERNUM AND (f2.FEETYPE like 'Freight Fee' OR f2.FEETYPE like 'Manual Freight Fee') AND ROWNUM = 1 AND ((UPPER(ADDRESS) NOT LIKE '%CASSELDEN%' Or UPPER(ADDRESS) NOT LIKE '2 LONSDALE%') OR (UPPER(ADDRESS2) NOT LIKE '%CASSELDEN%' Or UPPER(ADDRESS2) NOT LIKE '2 LONSDALE%')) --As "Line Charge"-- AND LAG(FEETYPE, 1, 0) OVER (ORDER BY FEETYPE) = 'Pick Fee'  THEN LEAD(SELLEXCL, 2, 0) OVER (ORDER BY SELLEXCL)
                    ELSE 0
                    END AS "Freight Charge"
      From TMP_ALL_FEES_F f1
	  ...
	  Select  f1.DESPDATE,f1.ORDERNUM,f1.DESPNOTE,f1.CUSTOMER,
        f1.ATTENTIONTO,Upper(f1.ADDRESS),Upper(f1.ADDRESS2),f1.SUBURB,f1.STATE,f1.POSTCODE,
        f1.ITEM,f1.DESCRIPTION,f1.QTY
             ,CASE    WHEN f1.FEETYPE like 'Stock' AND LAG(f1.DESPNOTE, 1, 0) OVER (ORDER BY f1.DESPNOTE) != f1.DESPNOTE THEN (Select f2.SELLEXCL From TMP_ALL_FEES_F f2 Where f2.ORDERNUM = f1.ORDERNUM AND f2.FEETYPE = 'Pick Fee' ) --As "Line Charge"-- AND LAG(FEETYPE, 1, 0) OVER (ORDER BY FEETYPE) = 'Pick Fee'  THEN LEAD(SELLEXCL, 2, 0) OVER (ORDER BY SELLEXCL)
                      ELSE 0
                      END AS "Line Charge"
            ,CASE     WHEN f1.FEETYPE like 'Stock' AND LAG(f1.DESPNOTE, 1, 0) OVER (ORDER BY f1.DESPNOTE) != f1.DESPNOTE 
                      THEN (
                            Select f2.SELLEXCL 
                            From TMP_ALL_FEES_F f2 
                            Where f2.ORDERNUM = f1.ORDERNUM 
                            AND f2.FEETYPE = 'Handeling Fee is ' 
                            ) --As "Line Charge"-- AND LAG(FEETYPE, 1, 0) OVER (ORDER BY FEETYPE) = 'Pick Fee'  THEN LEAD(SELLEXCL, 2, 0) OVER (ORDER BY SELLEXCL)
                      ELSE 0
                      END AS "Order Despatch Charge"
            ,CASE     WHEN f1.FEETYPE like 'Stock' AND (LAG(f1.DESPNOTE, 1, 0) OVER (ORDER BY f1.DESPNOTE) != f1.DESPNOTE)
                           AND InStr(UPPER(ADDRESS),'CASSELDEN') < 1 
                            AND InStr(UPPER(ADDRESS2),'2 LONSDALE') < 1 
                             --OR (InStr(UPPER(ADDRESS),'%CASSELDEN%') < 1 
                            --AND InStr(UPPER(ADDRESS2),'%2 LONSDALE%') < 1
                            --)
                            --AND ((UPPER(ADDRESS) NOT LIKE '%CASSELDEN%' Or UPPER(ADDRESS) NOT LIKE '%2 LONSDALE%') OR (UPPER(ADDRESS2) NOT LIKE '%CASSELDEN%' Or UPPER(ADDRESS2) NOT LIKE '%2 LONSDALE%'))
                      THEN  (
                          Select f2.SELLEXCL 
                          From TMP_ALL_FEES_F f2 
                          Where f2.ORDERNUM = f1.ORDERNUM 
                          AND (f2.FEETYPE like 'Freight Fee' OR f2.FEETYPE like 'Manual Freight Fee') 
                          AND ROWNUM = 1 
                          --AND ((UPPER(ADDRESS) NOT LIKE '%CASSELDEN%' Or UPPER(ADDRESS) NOT LIKE '%2 LONSDALE%') OR (UPPER(ADDRESS2) NOT LIKE '%CASSELDEN%' Or UPPER(ADDRESS2) NOT LIKE '%2 LONSDALE%'))
                          )
                      ELSE 0
...
Select  
  ...
  SELECT IM_CAT,COUNT(IM_CAT)
FROM IM 
WHERE IM_ACTIVE = 1
GROUP BY IM_CAT

SELECT IM_BRAND,COUNT(IM_BRAND)
FROM IM 
WHERE IM_ACTIVE = 1
GROUP BY IM_BRAND
