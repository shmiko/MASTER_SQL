...
exec :start_date := To_Date('1-Aug-2013')
var end_date varchar2(20)
exec :end_date := To_Date('31-Aug-2013')
...
/*Pallet In Fee*/
	...
	AND     IM_CUST IN (SELECT RM_CUST FROM RM WHERE RM_ANAL = :anal)
...
/*Stocks*/
	...
	SELECT    s.SH_CUST                AS "Customer",
	  ...
	  CASE    WHEN i.IM_CUST <> 'TABCORP' THEN s.SH_SPARE_STR_4
			  ...
			  s.SH_SPARE_STR_5         AS "OrderwareNum",
			  s.SH_CUST_REF            AS "CustomerRef",
			  l.vSLPickslipNum         AS "Pickslip",
			  NULL                     AS "PickNum",
			  l.vSLPslip               AS "DespatchNote",
			  l.vDateDespSL             AS "DespatchDate",
	  CASE    WHEN d.SD_STOCK IS NOT NULL THEN 'Stock'
			  ELSE NULL
			  END                      AS "FeeType",
			  d.SD_STOCK               AS "Item",
			  d.SD_DESC                AS "Description",
			  l.vSL_PICK_QTY           AS "Qty",
	  ...
	  CASE    WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> 'TABCORP' THEN To_Number(i.IM_REPORTING_PRICE)
			  ...
			  s.SH_NOTE_1              AS "DeliverTo",
			  s.SH_NOTE_2              AS "AttentionTo" ,
			  l.vWeightSL              AS "Weight",
			  l.vPackagesSL            AS "Packages",
			  s.SH_SPARE_DBL_9         AS "OrderSource",
			  NULL AS "Pallet/Shelf Space", /*Pallet/Space*/
				NULL AS "Locn", /*Locn*/
				0 AS "AvailSOH",/*Avail SOH*/
        ...
        CASE  WHEN regexp_substr(s.SH_SPARE_STR_3,'[a-z]+', 1, 2) IS NOT NULL THEN  s.SH_SPARE_STR_3 || '@' || s.SH_SPARE_STR_1
			        ...
			        END AS Email,
              i.IM_BRAND AS Brand
  FROM  Tmp_Admin_Data_Pick_LineCounts l INNER JOIN PWIN175.SH s ON  LTrim(s.SH_ORDER) = l.vSLOrderNum
	INNER JOIN PWIN175.RM r ON r.RM_CUST = s.SH_CUST
  INNER JOIN PWIN175.SD d ON d.SD_ORDER = s.SH_ORDER
			  --INNER JOIN PWIN175.ST t  ON t.ST_ORDER  = s.SH_ORDER
        --INNER JOIN PWIN175.SL l  ON l.SL_PICK   = t.ST_PICK
			  INNER JOIN PWIN175.IM i  ON i.IM_STOCK = d.SD_STOCK
        --INNER JOIN PWIN175.Tmp_Admin_Data_Pick_LineCounts l ON l.vSLPickslipNum =  t.ST_PICK
       -- INNER JOIN PWIN175.NI n  ON n.NI_NV_EXT_KEY = l.vSL_UID

  WHERE --NI_NV_EXT_TYPE = 1810105 AND NI_STRENGTH = 3 AND NI_DATE = t.ST_DESP_DATE AND NI_STOCK = d.SD_STOCK AND NI_STATUS <> 0
	--AND
  s.SH_STATUS <> 3
  --AND (r.RM_ANAL = :anal)
  AND r.RM_PARENT IN (SELECT RM_CUST FROM RM WHERE RM_ANAL = :anal)
	--AND       s.SH_ORDER = t.ST_ORDER
	AND       d.SD_STOCK NOT LIKE 'COURIER'
	AND       d.SD_STOCK NOT LIKE 'FEE*'
	AND       l.vDateDespSL >= :start_date AND l.vDateDespSL <= :end_date
	AND       d.SD_LAST_PICK_NUM = l.vSLPickslipNum
	GROUP BY  s.SH_CUST,
			  s.SH_NOTE_1,
			  s.SH_CAMPAIGN,
			  s.SH_SPARE_STR_4,
...
--	WHERE IM_CUST IN (SELECT RM_CUST FROM RM WHERE RM_ANAL = :anal)
	WHERE IM_ACTIVE = 1
	...
	AND n1.NI_STATUS <> 0
	GROUP BY IM_CUST,IM_XX_COST_CENTRE01,n1.NI_LOCN,n1.NI_AVAIL_ACTUAL,5,6,n1.NI_STOCK,8,9,10,11,12,l1.IL_NOTE_2,n1.NI_MADE_DATE,IM_LEVEL_UNIT,l1.IL_LOCN,nCountOfStocks,IM_BRAND
...
/*DB Maintenance Fee*/
	...
	AND    RM_PARENT IN (SELECT RM_CUST FROM RM WHERE RM_PARENT = ' '  AND RM_ANAL = :anal AND RM_TYPE = 0 AND RM_ACTIVE = 1 )
...
/*Stock Maint Charges including Stocktake and Kitting*/
	...
	AND     RM_CUST IN (SELECT RM_CUST FROM RM WHERE RM_ANAL = :anal)
  GROUP BY  RM_CUST
