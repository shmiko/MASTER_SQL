...
exec :start_date := To_Date('1-Jan-2015')
var end_date varchar2(20)
exec :end_date := To_Date('31-Jan-2015')
...
SELECT    s.SH_CUST                AS "Customer",
	   ...
	   CASE   WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN d.SD_SELL_PRICE
            ...
            WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) IS NOT NULL THEN eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK)
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) IS NULL THEN  d.SD_XX_OW_UNIT_PRICE
			      ELSE NULL
			      END                        AS "Batch/UnitPrice",
	   CASE   WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 THEN To_Number(i.IM_REPORTING_PRICE)
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 THEN  eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK)
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) IS NULL THEN  d.SD_XX_OW_UNIT_PRICE
			      ELSE NULL
			      END                        AS "OWUnitPrice",
      CASE  WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN d.SD_SELL_PRICE * l.SL_PSLIP_QTY
            ...
            WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) IS NOT NULL THEN  eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) * l.SL_PSLIP_QTY
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) IS NULL THEN  d.SD_XX_OW_UNIT_PRICE * l.SL_PSLIP_QTY
			      ELSE NULL
			      END          AS "DExcl",
	  ...
	  CASE    WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN (d.SD_SELL_PRICE * l.SL_PSLIP_QTY) * 1.1
            ...
            WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) IS NOT NULL THEN  (eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) * l.SL_PSLIP_QTY) * 1.1
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.sGroupCust,d.SD_STOCK) IS NULL THEN  (d.SD_XX_OW_UNIT_PRICE * l.SL_PSLIP_QTY) * 1.1
			      ELSE NULL
			      END          AS "DIncl",
	   CASE   WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN (d.SD_SELL_PRICE * l.SL_PSLIP_QTY) * 1.1
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 1 THEN  ((n.NI_SELL_VALUE/n.NI_NX_QUANTITY) * l.SL_PSLIP_QTY) * 1.1
