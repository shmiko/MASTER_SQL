...
var AdjustedDespDate varchar2(20)--exec :AdjustedDespDate := To_Date('28-Feb-2014')
...
--exec :AdjustedDespDate
            ...
            /*M,T,W,T*/
            , (CASE
                  WHEN substr(To_Char(h.SH_ADD_DATE, 'D'),0,10) IN (2,3,4,5)
                    AND h.SH_ADD_TIME < :CutOffOrderAddTime           --if order was in before cutoff
                    AND t.ST_DESP_DATE = h.SH_ADD_DATE            --If desp date was same day
                    AND t.ST_DESP_TIME <= :CutOffDespTimeSameDay  --If desp time was less 12 midnight  sameday
                  THEN  'SUCCESS/E/SD/<12/MTWT'
                  WHEN substr(To_Char(h.SH_ADD_DATE, 'D'),0,10) IN (2,3,4,5)
                        AND h.SH_ADD_TIME < :CutOffOrderAddTime           --if order was in before cutoff
                        AND t.ST_DESP_DATE > h.SH_ADD_DATE            --If desp date was after same day
                  ...
                  WHEN substr(To_Char(h.SH_ADD_DATE, 'D'),0,10) IN (2,3,4,5)
                        ...
                        AND t.ST_DESP_DATE > h.SH_ADD_DATE            --If desp date next day
                        AND t.ST_DESP_TIME <= :CutOffDespTimeSameDay  --If desp time was less that 12pm next day
                  THEN  'SUCCESS/L/ND/<12/MTWT'
                  WHEN substr(To_Char(h.SH_ADD_DATE, 'D'),0,10) IN (2,3,4,5)
                        AND h.SH_ADD_TIME > :CutOffOrderAddTime           --if order was in after cutoff
                        AND t.ST_DESP_DATE = h.SH_ADD_DATE            --If desp date same day
                  THEN  'SUCCESS/L/SD/MTWT'
                  WHEN substr(To_Char(h.SH_ADD_DATE, 'D'),0,10) IN (2,3,4,5)
                        AND h.SH_ADD_TIME > :CutOffOrderAddTime           --if order was in after cutoff
                        AND t.ST_DESP_DATE > h.SH_ADD_DATE            --If desp date after next day
                  THEN  'FAIL/L/>ND/MTWT'
                 -- WHEN weekday(To_Char(h.SH_ADD_DATE,Day)) IN (2,3,4,5)
                  --      AND t.ST_DESP_DATE - h.SH_ADD_DATE  >= 2
                 -- THEN  'FAIL/>2D/MTWT'
            ...
            AND h.SH_STATUS <> 3  AND h.SH_CAMPAIGN NOT IN( 'ADMIN','OBSOLETE') AND sGroupCust = 'TABCORP' --:cust
