 ...
 END GET_IAGRACV_PDS2;
  ...
  v_query           CLOB;
     ...
     v_query := q'{         SELECT DISTINCT SH_ORDER, 'TOTALS' AS "SCAN"
          ...
          LEFT JOIN SD SD1 ON SD1.SD_ORDER = SH_ORDER --INNER JOIN SD SD2 ON SD2.SD_LINE = SD1.SD_LINE
          INNER JOIN IM ON IM.IM_STOCK = SD1.SD_STOCK
        WHERE SH_ADD_DATE >= :iagracv_date_from_in AND SH_ADD_DATE <= :iagracv_date_to_in
        AND IM_CUST IN ('RACV','IAG')       --Just RACV or IAG
        AND SD1.SD_STATUS <> 3              --No cancelled lines
        AND F_GET_PDS_COUNTS(SH_ORDER) <> 0 --Must have at least 1 PDS Stock
        --AND SH_ORDER = '   1585603'         --Use a single order number for testing due to buffer output
...
BEGIN
  ...
  SELECT     SH.SH_CUST   AS "Customer"
           ...
           ,ST.ST_SPARE_DBL_1 As "PostWeight"
           ,SD.SD_SPARE_STR_4 As "BO Flag"
	FROM  PWIN175.SD
			  RIGHT JOIN PWIN175.SH  ON SH.SH_ORDER  = SD.SD_ORDER
			  LEFT JOIN PWIN175.ST  ON ST.ST_PICK  = SD.SD_LAST_PICK_NUM
        LEFT JOIN PWIN175.SL   ON SL.SL_PICK   = ST.ST_PICK
  ...
  AND       ST.ST_DESP_DATE >= gds_start_date_in AND ST.ST_DESP_DATE <= gds_end_date_in
	...
	GROUP BY  SH.SH_CUST,SH.SH_ORDER,SH.SH_ADD_DATE,SH.SH_ADDRESS,SH.SH_SUBURB,SH.SH_CITY,SH.SH_STATE,SH.SH_POST_CODE,SH.SH_NOTE_1,SH.SH_NOTE_2,SH.SH_CAMPAIGN, SH.SH_SPARE_STR_4,SH.SH_SPARE_DBL_9,SH.SH_CUST_REF,SH.SH_SPARE_STR_5,
			      ST.ST_ADD_DATE,ST.ST_ADD_OP,ST.ST_PICK_PRINT_DATE,ST.ST_DESP_DATE,ST.ST_WEIGHT,ST.ST_PACKAGES,ST.ST_PSLIP,ST.ST_PICK,ST.ST_SPARE_DBL_1,
			      SD.SD_DESC,SD.SD_STOCK,SD.SD_XX_OW_UNIT_PRICE,SD.SD_SELL_PRICE,SD.SD_LINE,SD.SD_EXCL,SD.SD_INCL,SD.SD_QTY_ORDER,SD.SD_QTY_UNIT,SD.SD_XX_PICKLIST_NUM,SD.SD_SPARE_STR_4,
  ...
  */
  ...
  PROCEDURE EOM_CREATE_TEMP_DATA_LOCATIONS 
    ...
    v_query := 'TRUNCATE TABLE Tmp_Locn_Cnt_By_Cust';
		...
		DBMS_OUTPUT.PUT_LINE('Successfully inserted fresh temporary location data');
