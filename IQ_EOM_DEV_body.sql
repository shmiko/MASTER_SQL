...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body IQ_EOM_DEV
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "PWIN175"."IQ_EOM_DEV" AS
      ...
      */
      EXECUTE IMMEDIATE 'INSERT into Dev_Group_Cust(sCust,sGroupCust,nLevel,AREA,TERR,RMDBL2,ANAL,SOURCE,OW_CAT )
                          SELECT RM_CUST
                            ...
                            ,LEVEL,RM_AREA,RM_TERR,(Select MAX(DV_VALUE) FROM DEV_DROP_LIST Where DV_INDEX = TO_NUMBER(RM_DBL_2)),RM_ANAL,RM_SOURCE,RM_GROUP_CUST
                      FROM RM
                      WHERE RM_TYPE = 0
                      AND RM_ACTIVE = 1
      ...
      EOM_INSERT_LOG(SYSTIMESTAMP ,NULL,NULL,'A_EOM_GROUP_CUST','RM','Dev_Group_Cust',v_time_taken,SYSTIMESTAMP,NULL);
    ...
    /*   Temp Tables Used   */
    /*   1. Dev_Admin_Data_BreakPrices   */
    /*   2. Dev_Admin_Data_Pickslips   */
    /*   3. Dev_Admin_Data_Pick_LineCounts   */
    /*   4. Dev_Batch_Price_SL_Stock   */
    ...
    BEGIN
      ...
      nCheckpoint := 5;
      --v_query := 'TRUNCATE TABLE Dev_Batch_Price_SL_Stock';
      --EXECUTE IMMEDIATE v_query;
      ...
      v_query := q'{INSERT INTO Dev_Admin_Data_Pick_LineCounts
        ...
        EXECUTE IMMEDIATE v_query USING start_date,end_date;
      /*nCheckpoint := 14;
      v_query := q'{INSERT INTO Dev_Batch_Price_SL_Stock(vBatchStock,vBatchPickNum,vUnitPrice,vDExcl, vQuantity)
     ...
     v_query2 :=  SQL%ROWCOUNT;
       ...
       -- ' Seconds...' ));
       v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
      IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,start_date,end_date,'B_EOM_START_RUN_ONCE_DATA','ST/SL','DEV_ADMIN_DATA_PICK_LINECOUNTS',v_time_taken,SYSTIMESTAMP,NULL);
     ...
     --If F_IS_TABLE_EEMPTY('DEV_ADMIN_DATA_PICK_LINECOUNTS') <= 0 Then
       ...
       --IQ_EOM_DEV.B_EOM_START_RUN_ONCE_DATA(start_date,end_date,sAnalysis_Start,sCust_start,0);
     -- End If;
   RETURN;
    EXCEPTION
      WHEN OTHERS THEN
    ...
    BEGIN
     nCheckpoint := 15;
      ...
      If F_IS_TABLE_EEMPTY('Dev_Locn_Cnt_By_Cust') <= 0 Then
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'C_EOM_START_ALL_TEMP_STOR_DATA','IL/NI','Dev_Locn_Cnt_By_Cust',v_time_taken,SYSTIMESTAMP,NULL);
    ...
    /*   F_EOM_DEV_ALL_FREIGHT_ALL Run this once for each customer   */
    /*   This gets all Freight data for queries   */
    /*   Temp Tables Used   */
    /*   1. DEV_ALL_FREIGHT_ALL   TO TEST AS MANUAL*/
    PROCEDURE F_EOM_DEV_ALL_FREIGHT_ALL (
      ...
      CURSOR c
      IS
        /* F_EOM_DEV_ALL_FREIGHT_ALL freight fees*/
    ...
    GROUP BY  
     ...
     BEGIN
      ...
      IF v_query2 > 0 THEN
          nCheckpoint := 100;
          v_query := '';
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','DEV_FREIGHT','DEV_ALL_FREIGHT_ALL',v_time_taken,SYSTIMESTAMP,NULL);--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
      ...
      --Else
        --DBMS_OUTPUT.PUT_LINE('F_EOM_DEV_ALL_FREIGHT_ALL rates are not empty - but there was no data, still took ' || (round((dbms_utility.get_time-l_start)/100, 6)) ||
       -- ' Seconds...for all customers ');
      END IF;
    EXCEPTION
      WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('F_EOM_DEV_ALL_FREIGHT_ALL failed at checkpoint ' || nCheckpoint ||
                            ' with error ' || SQLCODE || ' : ' || SQLERRM);
      RAISE;
    END F_EOM_DEV_ALL_FREIGHT_ALL;
     
    /*   F_EOM_DEV_MAN_FREIGHT_ALL Run this once for each customer   */
    /*   This gets all the IFS freight and Manual Freight data   */
    /*   Temp Tables Used   */
    /*   1. DEV_V_FREIGHT   TO TEST AS MANUAL*/
    PROCEDURE F_EOM_DEV_VAN_FREIGHT_ALL (
        p_array_size IN PLS_INTEGER DEFAULT 100
        ,startdate IN VARCHAR2-- := To_Date('1-Jun-2015') or format date as 01-Jun-15 -- use this when you want the date entered automatically
        ,enddate IN VARCHAR2-- := To_Date('30-Jun-2015')
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_V_FREIGHT%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
  ...
  --USING ;
    ...
    COMMIT;
      ...
      IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'F_EOM_DEV_VAN_FREIGHT_ALL','SD','DEV_V_FREIGHT',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('F_EOM_DEV_VAN_FREIGHT_ALL for the date range '
        --|| startdate || ' -- ' || enddate || ' - ' || v_query2
        --|| ' records inserted into table DEV_V_FREIGHT in ' || round((dbms_utility.get_time-l_start)/100, 6)
        --|| ' Seconds...for customer ' || sCustomerCode );
      --Else
        --DBMS_OUTPUT.PUT_LINE('F_EOM_DEV_VAN_FREIGHT_ALL rates are not empty - but there was no data, still took ' || (round((dbms_utility.get_time-l_start)/100, 6)) ||
       -- ' Seconds...for customer ' || sCustomerCode);
      END IF;
    EXCEPTION
      WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('F_EOM_DEV_VAN_FREIGHT_ALL failed at checkpoint ' || nCheckpoint ||
                            ' with error ' || SQLCODE || ' : ' || SQLERRM);
  
        RAISE;
  
    END F_EOM_DEV_VAN_FREIGHT_ALL;
    ...
    PROCEDURE F8_Z_EOM_RUN_FREIGHT (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_ALL_FREIGHT_F%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery         VARCHAR2(6000);
    ...
    COMMIT;
      ...
      IF v_query2 > 0 THEN
       -- If F_IS_TABLE_EEMPTY('DEV_FREIGHT') > 0 Then
          --sFileName := sCustomerCode || '-F8_Z_EOM_RUN_FREIGHT' || startdate || '-TO-' || enddate || '.csv';
          v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
          IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'F8_Z_EOM_RUN_FREIGHT','DEV_ALL_FREIGHT_ALL','DEV_ALL_FREIGHT_F',v_time_taken,SYSTIMESTAMP,sCustomerCode);
          sFileName := sCustomerCode || '-F8_Z_EOM_RUN_FREIGHT-' || startdate || '-TO-' || enddate || '-RunOn-' || sFileTime || '.csv';
        
          Z2_DEV_FEES_TO_CSV(sFileName,'DEV_ALL_FREIGHT_F');
          --DBMS_OUTPUT.PUT_LINE('Z2_DEV_FEES_TO_CSV for ' || sFileName || '.' );
        --End If;
    ...
    PROCEDURE H4_EOM_ALL_STOR_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_STOR_ALL_FEES%ROWTYPE;
      l_data ARRAY;
      v_out_tx          VARCHAR2(2000);
      v_time_taken VARCHAR2(205);
      SQLQuery   VARCHAR2(6000);
    ...
    EXECUTE IMMEDIATE v_query;
        ...
        If F_IS_TABLE_EEMPTY('DEV_STOR_ALL_FEES') > 0 Then
          ...
          v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
          IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'H4A_EOM_ALL_STOR_FEES','IL','DEV_STOR_ALL_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
          sFileName := sCustomerCode || '-H4A_EOM_ALL_STOR_FEES-' || startdate || '-TO-' || enddate || '-RunOn-' || sFileTime ||  '.csv';
          --Z2_DEV_FEES_TO_CSV(sFileName,'DEV_STOR_ALL_FEES');
          ----DBMS_OUTPUT.PUT_LINE('Z2_DEV_FEES_TO_CSV for ' || sFileName || '.' );
          --COMMIT;
          --DBMS_OUTPUT.PUT_LINE('H4A_EOM_ALL_STOR_FEES Fees for the date range '
              --|| startdate || ' -- ' || enddate || ' - ' || v_query2 || ' records inserted into table DEV_ALL_STOR_FEES in ' || (round((dbms_utility.get_time-l_start)/100, 6) ||
             -- ' Seconds...for customer ' || sCustomerCode ));
   ...
   -- sCust_Rates2 RM.RM_XX_FEE12%TYPE;
    ...
    EXECUTE IMMEDIATE v_query;
        ...
        --add another quick data check to ensure the source data is ok
          ...
          v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
          IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'H4_EOM_ALL_STOR','IL','DEV_STOR_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
          sFileName := sCustomerCode || '-H4_EOM_ALL_STOR-' || startdate || '-TO-' || enddate || '-RunOn-' || sFileTime ||  '.csv';
          Z2_DEV_FEES_TO_CSV(sFileName,'DEV_STOR_FEES');
          --DBMS_OUTPUT.PUT_LINE('Z2_DEV_FEES_TO_CSV for ' || sFileName || '.' );
          ...
          --DBMS_OUTPUT.PUT_LINE('H4_EOM_ALL_STOR Fees for the date range '
              --|| startdate || ' -- ' || enddate || ' - ' || v_query2 || ' records inserted into table DEV_STOR_FEES in ' || (round((dbms_utility.get_time-l_start)/100, 6) ||
             -- ' Seconds...for customer ' || sCustomerCode ));
    ...
    PROCEDURE E0_ALL_ORD_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_ALL_ORD_FEES%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
   ...
   --AND       t.ST_DESP_DATE >= startdate AND t.ST_DESP_DATE <= enddate
    ...
    GROUP BY   s.SH_CUST,r.sGroupCust,i.IM_CUST,s.SH_SPARE_STR_4,i.IM_XX_COST_CENTRE01,
     ...
     -- --DBMS_OUTPUT.PUT_LINE('aE1_.._ORD_FEES rates are $' || sCust_Rates1 || '. Prism rate field is RM_XX_FEE01.');
      ...
      IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'E0_ALL_ORD_FEES','SH','DEV_ALL_ORD_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('E0_ALL_ORD_FEES for the date range '
       -- || startdate || ' -- ' || enddate || ' - ' || v_query2
       -- || ' records inserted into table DEV_ALL_ORD_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
       -- || ' Seconds...for customer ' || sCustomerCode );
      --Else
  ...
  PROCEDURE E4_STD_ORD_FEES (
    ...
    IS
    TYPE ARRAY IS TABLE OF DEV_STD_ORD_FEES%ROWTYPE;
    l_data ARRAY;
    v_out_tx          VARCHAR2(2000);
    SQLQuery   VARCHAR2(6000);
    v_time_taken VARCHAR2(205);
...
*/
    ...
    IF v_query2 > 0 THEN
      v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
      IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'E4_VERBAL_ORD_FEES','SH','DEV_VERBAL_ORD_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
      DBMS_OUTPUT.PUT_LINE('E4_STD_ORD_FEES for the date range '
      || startdate || ' -- ' || enddate || ' - ' || v_query2
      || ' records inserted into table DEV_VERBAL_ORD_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
      || ' Seconds...for customer ' || sCustomerCode );
    Else
  ...
  END E4_STD_ORD_FEES;
    ...
    PROCEDURE E5_DESTOY_ORD_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_DESTROY_ORD_FEES%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
   ...
   -- SELECT To_Number(regexp_substr(RM_XX_FEE25,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCustomerCode) > 0
    ...
    AND       t.ST_DESP_DATE >= startdate AND t.ST_DESP_DATE <= enddate;
      ...
      IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'E5_DESTOY_ORD_FEES','SH','DEV_DESTROY_ORD_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('E5_DESTOY_ORD_FEES for the date range '
       -- || startdate || ' -- ' || enddate || ' - ' || v_query2
       -- || ' records inserted into table DEV_DESTROY_ORD_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
       -- || ' Seconds...for customer ' || sCustomerCode );
      --Else
   ...
   ---- Else
    ...
    /*   Temp Tables Used   */
    /*   1. DEV_SHRINKWRAP_FEES  ShrinkWrap Fee */
    /*   Prism Rate Field Used   */
    /*   A. RM_XX_FEE18   */
    PROCEDURE G1_SHRINKWRAP_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_SHRINKWRAP_FEES%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
  ...
  --	AND       TO_CHAR(t.ST_DESP_DATE,'YYYY-MM-DD') >= start_date AND TO_CHAR(t.ST_DESP_DATE,'YYYY-MM-DD') <= end_date
    ...
    GROUP BY  s.SH_CUST,
     ...
     BEGIN
      ...
      COMMIT;
       ...
       IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'G1_SHRINKWRAP_FEES','ST','DEV_SHRINKWRAP_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('G1_SHRINKWRAP_FEES for the date range '
       -- || startdate || ' -- ' || enddate || ' - ' || v_query2
       -- || ' records inserted into table DEV_SHRINKWRAP_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
       -- || ' Seconds...for customer ' || sCustomerCode );
      --Else
    ...
    PROCEDURE G2_STOCK_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_STOCK_FEES%ROWTYPE;
      l_data ARRAY;
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
      v_time_taken VARCHAR2(205);
     ...
     BEGIN
      ...
      COMMIT;
       ...
       IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'G2_STOCK_FEES','SD','DEV_STOCK_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('G2_STOCK_FEES for the date range '
        --|| startdate || ' -- ' || enddate || ' - ' || v_query2
       -- || ' records inserted into table DEV_STOCK_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
       -- || ' Seconds...for customer ' || sCustomerCode );
      --Else
    ...
    /*   This gets all the Handeling Related Data   */
    /*   Temp Tables Used   */
    /*   1. DEV_PACKING_FEES   Packing Fee  */
    /*   Prism Rate Field Used   */
    /*   A. RM_XX_FEE08 1   */
    PROCEDURE G3_PACKING_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_PACKING_FEES%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
    ...
    l_start number default dbms_utility.get_time;
     BEGIN
      ...
      COMMIT;
          ...
          IF v_query2 > 0 THEN
            v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
            IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'G3_PACKING_FEES','SL','DEV_PACKING_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
            --DBMS_OUTPUT.PUT_LINE('G3_PACKING_FEES for the date range '
            --|| startdate || ' -- ' || enddate || ' - ' || v_query2
           -- || ' records inserted into table DEV_PACKING_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
           -- || ' Seconds...for customer ' || sCustomerCode );
          --Else
    ...
    /*   This gets all the Handeling Related Data   */
    /*   Temp Tables Used   */
    /*   1. DEV_HANDLING_FEES   Handling Fee  */
    /*   Prism Rate Field Used   */
    /*   A. RM_XX_FEE06   */
    PROCEDURE G4_HANDLING_FEES_F (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_HANDLING_FEES%ROWTYPE;
      l_data ARRAY;
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
      v_time_taken VARCHAR2(205);
    ...
    l_start number default dbms_utility.get_time;
     BEGIN
      ...
      COMMIT;
         ...
         IF v_query2 > 0 THEN
            v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
            IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'G4_HANDLING_FEES_F','SL','DEV_HANDLING_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
            --DBMS_OUTPUT.PUT_LINE('G4_HANDLING_FEES_F for the date range '
           -- || startdate || ' -- ' || enddate || ' - ' || v_query2
           -- || ' records inserted into table DEV_HANDLING_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
           -- || ' Seconds...for customer ' || sCustomerCode );
          --Else
    ...
    PROCEDURE G4_HANDLING_FEES_F2 (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_PICK_FEES%ROWTYPE;
      l_data ARRAY;
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
      v_time_taken VARCHAR2(205);
    ...
    l_start number default dbms_utility.get_time;
     BEGIN
      ...
      COMMIT;
         ...
         IF v_query2 > 0 THEN
            v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
            IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'G4_HANDLING_FEES_F2','SL','DEV_PICK_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
            --DBMS_OUTPUT.PUT_LINE('G4_HANDLING_FEES_F2 for the date range '
           -- || startdate || ' -- ' || enddate || ' - ' || v_query2
           -- || ' records inserted into table DEV_PICK_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
           -- || ' Seconds...for customer ' || sCustomerCode );
          --Else
  ...
  AND t.ST_DESP_DATE >= startdate AND t.ST_DESP_DATE <= enddate
     ...
     BEGIN
      ...
      IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'G5_PICK_FEES_F','ST','DEV_PICK_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
  ...
  END G5_PICK_FEES_F2;
    ...
    PROCEDURE I_EOM_MISC_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_MISC_FEES%ROWTYPE;
      l_data ARRAY;
      v_out_tx          VARCHAR2(2000);
      v_time_taken VARCHAR2(205);
      SQLQuery   VARCHAR2(6000);
  ...
  ;
    ...
    COMMIT;
      ...
      IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'I_EOM_MISC_FEES','RM','DEV_MISC_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('I_EOM_MISC_FEES for the date range '
       -- || startdate || ' -- ' || enddate || ' - ' || v_query2
       -- || ' records inserted into table DEV_MISC_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
       -- || ' Seconds...for customer ' || sCustomerCode );
      --Else
    ...
    PROCEDURE J_EOM_CUSTOMER_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_CUSTOMER_FEES%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
  ...
  /*BB PackingFee*/
   ...
   SELECT
    ...
    --RETURN;
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'J_EOM_CUSTOMER_FEES','RM','DEV_CUSTOMER_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
   ...
   /*    UNION ALL
    ...
    COMMIT;
     ...
     IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'J_EOM_CUSTOMER_FEES_TAB','RM','DEV_CUSTOMER_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('J_EOM_CUSTOMER_FEES_TAB for the date range '
       -- || startdate || ' -- ' || enddate || ' - ' || v_query2
       -- || ' records inserted into table DEV_CUSTOMER_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
       -- || ' Seconds...for customer ' || sCustomerCode );
      --Else
  ...
  --	AND       IM_MAIN_SUPP <> 'BSPGA'
    ...
    IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'J_EOM_CUSTOMER_FEES_BB','RM','DEV_CUSTOMER_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('J_EOM_CUSTOMER_FEES_BB for the date range '
        --|| startdate || ' -- ' || enddate || ' - ' || v_query2
        --|| ' records inserted into table DEV_CUSTOMER_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
        --|| ' Seconds...for customer ' || sCustomerCode );
   ...
   ;
    ...
    IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'J_EOM_CUSTOMER_FEES_VHA','RM','DEV_CUSTOMER_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
    ...
    PROCEDURE K1_PAL_DESP_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_PAL_DESP_FEES%ROWTYPE;
      l_data ARRAY;
      v_out_tx          VARCHAR2(2000);
      v_time_taken VARCHAR2(205);
      SQLQuery          VARCHAR2(6000);
    ...
    COMMIT;
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'K1_PAL_DESP_FEES','ST','DEV_PAL_DESP_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('K1_PAL_DESP_FEES for the date range '
          --|| startdate || ' -- ' || enddate || ' - ' || v_query2
         -- || ' records inserted into table DEV_PAL_CTN_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
         -- || ' Seconds...for customer ' || sCustomerCode );
       --Else
    ...
    PROCEDURE K2_CTN_IN_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_CTN_IN_FEES%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
  ...
  --	AND       IM_MAIN_SUPP <> 'BSPGA'
    ...
    COMMIT;
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'K2_CTN_IN_FEES','ST','DEV_CTN_IN_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('K2_CTN_IN_FEES for the date range '
         -- || startdate || ' -- ' || enddate || ' - ' || v_query2
         -- || ' records inserted into table DEV_CTN_IN_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
         -- || ' Seconds...for customer ' || sCustomerCode );
       --Else
    ...
    PROCEDURE K3_PAL_IN_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_PAL_IN_FEES%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
  ...
  --	AND       IM_MAIN_SUPP <> 'BSPGA'
    ...
    COMMIT;
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'K3_PAL_IN_FEES','ST','DEV_PAL_IN_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('K3_PAL_IN_FEES for the date range '
         -- || startdate || ' -- ' || enddate || ' - ' || v_query2
         -- || ' records inserted into table DEV_PAL_IN_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
         -- || ' Seconds...for customer ' || sCustomerCode );
       --Else
  ...
  --	AND       IM_MAIN_SUPP <> 'BSPGA'
    ...
    COMMIT;
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'K3_ALL_PAL_IN_FEES','ST','DEV_PAL_IN_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
        --DBMS_OUTPUT.PUT_LINE('K3_PAL_IN_FEES for the date range '
         -- || startdate || ' -- ' || enddate || ' - ' || v_query2
         -- || ' records inserted into table DEV_PAL_IN_FEES in ' || round((dbms_utility.get_time-l_start)/100, 6)
         -- || ' Seconds...for customer ' || sCustomerCode );
    ...
    PROCEDURE K4_CTN_DESP_FEES (
      ...
      IS
      TYPE ARRAY IS TABLE OF DEV_CTN_DESP_FEES%ROWTYPE;
      v_time_taken VARCHAR2(205);
      l_data ARRAY;
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
  ...
  /*Carton Despatch Fee*/
   ...
   (SELECT To_Number(regexp_substr(RM_XX_FEE15,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCustomerCode) > 0
    ...
    v_query2 :=  SQL%ROWCOUNT;
      ...
      v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
      IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'K4_CTN_DESP_FEES','ST','DEV_CTN_DESP_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
      --DBMS_OUTPUT.PUT_LINE('K4_CTN_DESP_FEES for the date range '
     -- || startdate || ' -- ' || enddate || ' - ' || v_query2
     -- || ' records inserted into table DEV_CTN_DESP_FEES in ' || (round((dbms_utility.get_time-l_start)/100, 6)
     -- || ' Seconds...for customer ' || sCustomerCode ));
    --Else
    ...
    AS
      TYPE ARRAY IS TABLE OF DEV_DESP_REPT%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
     ...
     OPEN c;
      ...
      COMMIT; 
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
          IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,gds_start_date_in,gds_end_date_in,'L_DESPATCH_REPORT','DEV_DESP_REPT','ST',v_time_taken,SYSTIMESTAMP,'LINK');
          sFileName := 'LINK-L_DESPATCH_REPORT-' || gds_start_date_in || '-TO-' || gds_end_date_in || '-RunOn-' || sFileTime || '_A.csv';
          IQ_EOM_DEV.L_DESPATCH_REPORTB(p_array_size,gds_analysis,gds_next_start_date_in,gds_next_end_date_in);
          Z2_DEV_FEES_TO_CSV(sFileName,'DEV_DESP_REPT');
          --DBMS_OUTPUT.PUT_LINE('DEV_DESP_REPT_TO_CSV for ' || sFileName || '.' );
     EXCEPTION
        WHEN OTHERS THEN
          DBMS_OUTPUT.PUT_LINE('L_DESPATCH_REPORT failed at checkpoint ' || nbreakpoint ||
                              ' with error ' || SQLCODE || ' : ' || SQLERRM);
          RAISE;
    ...
    AS
      TYPE ARRAY IS TABLE OF DEV_DESP_REPT2%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
     ...
     OPEN c;
      ...
      COMMIT; 
      v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
          IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,gds_start_date_in,gds_end_date_in,'L_DESPATCH_REPORTB','DEV_DESP_REPT2','ST',v_time_taken,SYSTIMESTAMP,'LINK');
          sFileName := 'LINK-L_DESPATCH_REPORTB-' || gds_start_date_in || '-TO-' || gds_end_date_in || '-RunOn-' || sFileTime || '_B.csv';
        
          Z2_DEV_FEES_TO_CSV(sFileName,'DEV_DESP_REPT2');
          --DBMS_OUTPUT.PUT_LINE('DEV_DESP_REPT_TO_CSV B for ' || sFileName || '.' );
     EXCEPTION
        WHEN OTHERS THEN
    ...
    /* Y Run this once for each customer including intercompany   
       ...
       Temp Tables Used   
       1. DEV_ALL_FEES   */
    PROCEDURE Y_EOM_DEV_MERGE_ALL_FEES (
     p_array_size IN PLS_INTEGER DEFAULT 100
     )
      IS
      TYPE ARRAY IS TABLE OF DEV_ALL_FEES_F%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
     ...
     -- end_date2 ST.ST_DESP_DATE%TYPE := end_date;
      ...
      --sCust IN RM.RM_CUST%TYPE
       --)
       IS
            --Insert Into DEV_ALL_FEES
            --Select * From DEV_FREIGHT
            --UNION ALL
            Select * From DEV_V_FREIGHT
            UNION ALL
            --Select * From DEV_M_XX_FREIGHT
            --UNION ALL          
           -- Select * From DEV_ALL_FREIGHT_F WHERE FEETYPE != 'UnPricedManualFreight'-- AND Trim(FEETYPE)  != 'Freight Fee'
           -- UNION ALL
           -- Select * From DEV_ALL_FREIGHT_F
            --WHERE Trim(FEETYPE) = 'Freight Fee' AND rowid in
            --(select max(rowid) from DEV_ALL_FREIGHT_F WHERE Trim(FEETYPE)  = 'Freight Fee' group by DESPNOTE)
            --UNION ALL
            Select  *
            ...
            UNION ALL --DEV_ALL_ORD_FEES
            Select * From DEV_ALL_ORD_FEES
            
            UNION ALL
            Select * From DEV_STD_ORD_FEES
           /* UNION ALL
            Select * From DEV_FAX_ORD_FEES
            UNION ALL
            Select * From DEV_MAN_ORD_FEES
            UNION ALL
            Select * From DEV_EMAIL_ORD_FEES
            UNION ALL,          
            */
            UNION ALL         
            ...
            --UNION ALL
            --Select * From DEV_SLOW_STOR_FEES
            --UNION ALL
            --Select * From DEV_SEC_STOR_FEES
    ...
    --RETURN;
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Y_EOM_DEV_MERGE_ALL_FEES','TMP','DEV_ALL_FEES_F',v_time_taken,SYSTIMESTAMP,NULL);
        
    --DBMS_OUTPUT.PUT_LINE('Y_EOM_DEV_MERGE_ALL_FEES and dump data in ' ||  (round((dbms_utility.get_time-l_start)/100, 6) ||
     --   ' Seconds...' ));
  
      COMMIT;
    EXCEPTION
      WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Y_EOM_DEV_MERGE_ALL_FEES failed at checkpoint ' || nCheckpoint ||
                            ' with error ' || SQLCODE || ' : ' || SQLERRM);
        RAISE;
  
    END Y_EOM_DEV_MERGE_ALL_FEES;
    
    /* Y Run this once for each customer including intercompany   
       This merges all the Charges from each of the temp tables   
       Temp Tables Used   
       1. DEV_ALL_FEES   */
    PROCEDURE Y_EOM_DEV_MERGE_ALL_FEES2 
      IS
      TYPE ARRAY IS TABLE OF DEV_ALL_FEES%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
      v_out_tx          VARCHAR2(2000);
      SQLQuery   VARCHAR2(6000);
     ...
     -- end_date2 ST.ST_DESP_DATE%TYPE := end_date;
      ----DBMS_OUTPUT.PUT_LINE(start_date || ' - ' || end_date || '.' || sCust );
       l_start number default dbms_utility.get_time;
  BEGIN
   v_query := q'{INSERT INTO DEV_ALL_FEES
      Select  *
                From DEV_ALL_FREIGHT_F 
                WHERE FEETYPE = 'Freight Fee' AND rowid in
  ...
  nCheckpoint := 2;
    ...
    --RETURN;
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Y_EOM_DEV_MERGE_ALL_FEES2','TMP','DEV_ALL_FEES',v_time_taken,SYSTIMESTAMP,NULL);
        
    --DBMS_OUTPUT.PUT_LINE('Y_EOM_DEV_MERGE_ALL_FEES2 and dump data in ' ||  (round((dbms_utility.get_time-l_start)/100, 6) ||
      --  ' Seconds...' ));
  
      COMMIT;
    EXCEPTION
      WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Y_EOM_DEV_MERGE_ALL_FEES2 failed at checkpoint ' || nCheckpoint ||
                            ' with error ' || SQLCODE || ' : ' || SQLERRM);
        RAISE;
  
    END Y_EOM_DEV_MERGE_ALL_FEES2;
    
    /* Y Run this once for each customer including intercompany   
       This merges all the Charges from each of the temp tables   
       Temp Tables Used   
       1. DEV_ALL_FEES   */
    PROCEDURE Y_EOM_DEV_MERGE_ALL_FEES_FINAL(sCustomerCode IN VARCHAR2) 
      IS
      TYPE ARRAY IS TABLE OF DEV_ALL_FEES_F%ROWTYPE;
      l_data ARRAY;
      v_time_taken VARCHAR2(205);
  ...
  nCheckpoint := 2;
    ...
    --RETURN;
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Y_EOM_DEV_MERGE_ALL_FEES_FINAL','DEV_ALL_FEES','DEV_ALL_FEES_F',v_time_taken,SYSTIMESTAMP,NULL);
        
    --DBMS_OUTPUT.PUT_LINE('Y_EOM_DEV_MERGE_ALL_FEES_FINAL and dump data in ' ||  (round((dbms_utility.get_time-l_start)/100, 6) ||
      --  ' Seconds...' )); 
      COMMIT;
    EXCEPTION
      WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Y_EOM_DEV_MERGE_ALL_FEES_FINAL failed at checkpoint ' || nCheckpoint ||
                            ' with error ' || SQLCODE || ' : ' || SQLERRM);
        RAISE;
  
    END Y_EOM_DEV_MERGE_ALL_FEES_FINAL;
  ...
  BEGIN
    ...
    nCheckpoint := 2;
      ...
      EXECUTE IMMEDIATE v_query;
      --Select (F_EOM_CHECK_LOG(v_tmp_date ,'Dev_Group_Cust','A_EOM_GROUP_CUST')) INTO v_query_logfile From Dual;--v_query := q'{Select IQ_EOM_DEV.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL') }';--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
      --If UPPER(v_query_logfile) != UPPER(v_tmp_date) Then
      If F_IS_TABLE_EEMPTY('Dev_Group_Cust') <= 0 Then
        --DBMS_OUTPUT.PUT_LINE('1st Need to run Dev_Group_Cust for all customers as table is empty.' );
        A_EOM_GROUP_CUST();
      --Else
        --DBMS_OUTPUT.PUT_LINE('1st No Need to run Dev_Group_Cust for all customers as table is full of data - saved another 5 seconds.' );
      End If;
    
    nCheckpoint := 3;
      --v_query := q'{SELECT TO_CHAR(LAST_ANALYZED, 'DD-MON-YY') FROM DBA_TABLES WHERE TABLE_NAME = 'DEV_ADMIN_DATA_PICK_LINECOUNTS'}';
      --EXECUTE IMMEDIATE v_query INTO vRtnVal;-- USING sCustomerCode;
      --If F_IS_TABLE_EEMPTY('DEV_ADMIN_DATA_PICK_LINECOUNTS') <= 0 Then
      Select (F_EOM_CHECK_LOG(v_tmp_date ,'DEV_ADMIN_DATA_PICK_LINECOUNTS','B_EOM_START_RUN_ONCE_DATA')) INTO v_query_logfile From Dual;--v_query := q'{Select IQ_EOM_DEV.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL') }';--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
      If F_IS_TABLE_EEMPTY('DEV_ADMIN_DATA_PICK_LINECOUNTS') <= 0 Then
        --DBMS_OUTPUT.PUT_LINE('2nd Need to RUN_ONCE DEV_ADMIN_DATA_PICK_LINECOUNTS as B_EOM_START_RUN_ONCE_DATA for all customers as table is empty. result was ' || UPPER(v_query_logfile) || ' and end date was ' ||  UPPER(v_tmp_date) );
      ...
      ELSIf UPPER(v_query_logfile) != UPPER(v_tmp_date) Then
        ...
        B_EOM_START_RUN_ONCE_DATA(start_date,end_date,sAnalysis_Start,sCust_start,0);
      --Else
        --DBMS_OUTPUT.PUT_LINE('2nd No Need to RUN_ONCE DEV_ADMIN_DATA_PICK_LINECOUNTS as B_EOM_START_RUN_ONCE_DATA for all customers as table is full of data - saved another 45 seconds. Last Date match was ' || UPPER(v_query_logfile) || ' and end date was ' ||  UPPER(v_tmp_date) );
      End If;
    
    nCheckpoint := 4;
    --set timing on; Dev_Locn_Cnt_By_Cust
    If F_IS_TABLE_EEMPTY('Dev_Locn_Cnt_By_Cust') <= 0 Then
   -- Select (F_EOM_CHECK_LOG(v_tmp_date ,'Dev_Locn_Cnt_By_Cust','C_EOM_START_ALL_TEMP_STOR_DATA')) INTO v_query_logfile From Dual;--v_query := q'{Select IQ_EOM_DEV.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL') }';--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
    --If UPPER(v_query_logfile) != UPPER(v_tmp_date) OR F_IS_TABLE_EEMPTY('Dev_Locn_Cnt_By_Cust') <= 0 Then
       --DBMS_OUTPUT.PUT_LINE('3rd Need to RUN_ONCE Dev_Locn_Cnt_By_Cust as C_EOM_START_ALL_TEMP_STOR_DATA for all customers as table is empty.result was ' || UPPER(v_query_logfile) || ' and end date was ' ||  UPPER(v_tmp_date) );
     --IQ_EOM_DEV.C_EOM_START_CUST_TEMP_DATA(sAnalysis_Start,sCust_start);
     C_EOM_START_ALL_TEMP_STOR_DATA(sAnalysis_Start,sCust_start);
   -- Else
      --DBMS_OUTPUT.PUT_LINE('3rd No Need to RUN_ONCE Dev_Locn_Cnt_By_Cust as C_EOM_START_ALL_TEMP_STOR_DATA for all customers as table is full of data - saved another 65 seconds. Last Date match was ' || UPPER(v_query_logfile) || ' and end date was ' ||  UPPER(v_tmp_date) );
    End If;
    ...
    nCheckpoint := 5;
      ...
      Select (F_EOM_CHECK_LOG(v_tmp_date ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL')) INTO v_query_logfile From Dual;--v_query := q'{Select IQ_EOM_DEV.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL') }';--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
      Select (F_EOM_CHECK_CUST_LOG(sCust_start ,'DEV_ALL_FREIGHT_F','F8_Z_EOM_RUN_FREIGHT')) INTO v_query_result2 From Dual;--v_query := q'{Select IQ_EOM_DEV.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL') }';--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
      If F_IS_TABLE_EEMPTY('DEV_ALL_FREIGHT_ALL') <= 0 Then
        --DBMS_OUTPUT.PUT_LINE(''
         -- || '5th this suggests that BOTH the bulk freight data AND the customer freight data is NOT fresh - re run both!!!'
         -- || ' Need to RUN_ONCE F_EOM_DEV_ALL_FREIGHT_ALL 1 for all customers as table is empty.'
         ...
         -- ); --F_EOM_DEV_VAN_FREIGHT_ALL
        --IQ_EOM_DEV.F_EOM_DEV_VAN_FREIGHT_ALL(p_array_size_start,start_date,end_date,sCust_start);
        IQ_EOM_DEV.F_EOM_DEV_ALL_FREIGHT_ALL(p_array_size_start,start_date,end_date);
        IQ_EOM_DEV.F8_Z_EOM_RUN_FREIGHT(p_array_size_start,start_date,end_date,sCust_start,sFilterBy);
      ...
      Then
        --DBMS_OUTPUT.PUT_LINE(''
        --  || '5th this suggests that BOTH the bulk freight data AND the customer freight data is NOT fresh - re run both!!!'
        --  || ' Need to RUN_ONCE F_EOM_DEV_ALL_FREIGHT_ALL 1 for all customers as table is not empty.'
        --  || ' Last Cust match was ' ||  UPPER(v_query_result2) 
        --  || ' and this cust was ' ||  UPPER(sCust_start)
        --  || ' and to date was ' ||  UPPER(v_query_logfile)
         -- || ' and this date was ' ||  UPPER(v_tmp_date) 
      ...
      --  AND UPPER(v_query_logfile) IS NOT NULL 
       ...
       -- Then
         ...
         -- || ' No Need to RUN_ONCE F_EOM_DEV_ALL_FREIGHT_ALL 1 for all customers as table is not empty.'
         -- || ' Last Cust match was ' ||  UPPER(v_query_result2) 
         -- || ' and this cust was ' ||  UPPER(sCust_start)
         -- || ' and to date was ' ||  UPPER(v_query_logfile)
         -- || ' and this date was ' ||  UPPER(v_tmp_date) 
        ...
        Then
         ...
         -- || ' Need to RUN_ONCE F_EOM_DEV_ALL_FREIGHT_ALL 1 for all customers as table is from different dates and different customer.' 
        ...
        Then
         ...
         -- || '5th this suggests that the bulk freight data is fresh BUT the customer freight data is NOT fresh - re run just cust data!!!'
          --|| ' Need to RUN_ONCE F_EOM_DEV_ALL_FREIGHT_ALL for all customers as table is from different dates and different customer.' 
          --|| ' Last Cust match was ' ||  UPPER(v_query_result2) 
         -- || ' and this cust was ' ||  UPPER(sCust_start)
         -- || ' and to date was ' ||  UPPER(v_query_logfile)
         -- || ' and this date was ' ||  UPPER(v_tmp_date) 
        ...
        --DBMS_OUTPUT.PUT_LINE(''
         ...
         -- );
          ...
          IQ_EOM_DEV.F8_Z_EOM_RUN_FREIGHT(p_array_size_start,start_date,end_date,sCust_start,sFilterBy);      
       -- Else
          --DBMS_OUTPUT.PUT_LINE('5th No matches for running F_EOM_DEV_ALL_FREIGHT_ALL 1,' 
        --  || 'still took ' || (round((dbms_utility.get_time-l_start)/100, 6)) || 'Seconds...'
         -- || ' for customer ' || sCust_start);
        End If;
     
      nCheckpoint := 6;
        Select (F_EOM_CHECK_LOG(v_tmp_date ,'DEV_STOR_ALL_FEES','H4_EOM_ALL_STOR_FEES')) INTO v_query_logfile From Dual;--v_query := q'{Select IQ_EOM_DEV.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL') }';--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
        Select (F_EOM_CHECK_CUST_LOG(sCust_start ,'DEV_STOR_FEES','H4_EOM_ALL_STORS')) INTO v_query_result2 From Dual;--v_query := q'{Select IQ_EOM_DEV.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL') }';--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
        If F_IS_TABLE_EEMPTY('DEV_STOR_ALL_FEES') <= 0 Then
          --DBMS_OUTPUT.PUT_LINE(''
         -- || '6th this suggests that BOTH the bulk storage data AND the customer storage data is NOT fresh - re run both!!!'
         -- || '  Need to RUN_ONCE H4_EOM_ALL_STOR_FEES 1 for all customers as table is empty. Last Cust match was ' ||  UPPER(v_query_logfile) 
         -- || ' and this cust was ' ||  UPPER(sCust_start)
         -- || ' and to date was ' ||  UPPER(v_query_result2)
         -- || ' and this date was ' ||  UPPER(v_tmp_date) 
         -- );
          IQ_EOM_DEV.H4_EOM_ALL_STOR_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);
          IQ_EOM_DEV.H4_EOM_ALL_STOR(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
        ELSIf UPPER(v_query_logfile) != UPPER(v_tmp_date) 
        AND UPPER(v_query_logfile) IS NOT NULL
        AND UPPER(v_query_result2) != UPPER(sCust_start) 
        Then
          ...
          --DBMS_OUTPUT.PUT_LINE(''
        --  || '6th this suggests that BOTH the bulk storage data AND the customer storage data is NOT fresh - re run both!!!'
        --  || ' Need to RUN_ONCE H4_EOM_ALL_STOR_FEES 1 for all customers as table is not empty. Last Cust match was ' ||  UPPER(v_query_logfile) 
        --  || ' and this cust was ' ||  UPPER(sCust_start)
        --  || ' and to date was ' ||  UPPER(v_query_result2)
        --  || ' and this date was ' ||  UPPER(v_tmp_date) 
        --  );
       ...
       -- AND UPPER(v_query_logfile) = UPPER(v_tmp_date) 
        --Then
         ...
         ---- || '  No Need to RUN_ONCE H4_EOM_ALL_STOR_FEES 1 for all customers as table is not empty. Last Cust match was ' ||  UPPER(v_query_logfile) 
         -- || ' and this cust was ' ||  UPPER(sCust_start)
        --  || ' and to date was ' ||  UPPER(v_query_result2)
        --  || ' and this date was ' ||  UPPER(v_tmp_date) 
        --  );
        ...
        Then
         ...
         -- || '  Need to RUN_ONCE H4_EOM_ALL_STOR_FEES 1 for all customers as table is from different dates and different customer. Last Cust match was ' ||  UPPER(v_query_logfile) 
         -- || ' and this cust was ' ||  UPPER(sCust_start)
         -- || ' and to date was ' ||  UPPER(v_query_result2)
         -- || ' and this date was ' ||  UPPER(v_tmp_date) 
         -- );
        ...
        --DBMS_OUTPUT.PUT_LINE(''
         ...
         -- );
          ...
          IQ_EOM_DEV.H4_EOM_ALL_STOR(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);  
        --Else
          --DBMS_OUTPUT.PUT_LINE('6th No matches for running H4_EOM_ALL_STOR_FEES 1,' 
         -- || 'still took ' || (round((dbms_utility.get_time-l_start)/100, 6)) || 'Seconds...'
         -- || ' for customer ' || sCust_start);
        End If;
        
      nCheckpoint := 71; --E0_ALL_ORD_FEES
      IQ_EOM_DEV.E0_ALL_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      
    /*IQ_EOM_DEV.E1_PHONE_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);
      nCheckpoint := 72;
      IQ_EOM_DEV.E2_EMAIL_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);
      nCheckpoint := 73;
      IQ_EOM_DEV.E3_FAX_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);
     */  
      nCheckpoint := 74;
      IQ_EOM_DEV.E4_STD_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
     
      nCheckpoint := 75;
      IQ_EOM_DEV.E5_DESTOY_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);

      nCheckpoint := 81;
      IQ_EOM_DEV.G1_SHRINKWRAP_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      nCheckpoint := 82;
      IQ_EOM_DEV.G2_STOCK_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      --nCheckpoint := 83;
      --IQ_EOM_DEV.G3_PACKING_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      
      
      nCheckpoint := 84;
      Select (F_EOM_CHECK_CUST_LOG(sCust_start ,'DEV_HANDLING_FEES','G4_HANDLING_FEES_F')) INTO v_query_result2 From Dual;
      Select (F_EOM_CHECK_LOG(v_tmp_date ,'DEV_HANDLING_FEES','G4_HANDLING_FEES_F')) INTO v_query_logfile From Dual;
      If F_IS_TABLE_EEMPTY('DEV_HANDLING_FEES') <= 0 Then
        ...
        --|| ' and this date was ' ||  UPPER(v_tmp_date) 
       -- );
        IQ_EOM_DEV.G4_HANDLING_FEES_F(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      ELSIf UPPER(v_query_result2) != UPPER(sCust_start) 
      AND UPPER(v_query_logfile) IS NOT NULL 
      ...
      nCheckpoint := 85;
      Select (F_EOM_CHECK_CUST_LOG(sCust_start ,'DEV_PICK_FEES','G5_PICK_FEES_F')) INTO v_query_result2 From Dual;--v_query := q'{Select IQ_EOM_DEV.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'DEV_ALL_FREIGHT_ALL','F_EOM_DEV_ALL_FREIGHT_ALL') }';--q'{INSERT INTO DEV_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_DEV_ALL_FREIGHT_ALL','NONE','DEV_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
      Select (F_EOM_CHECK_LOG(v_tmp_date ,'DEV_PICK_FEES','G5_PICK_FEES_F')) INTO v_query_logfile From Dual;
      If F_IS_TABLE_EEMPTY('DEV_PICK_FEES') <= 0 Then
       ...
       -- || ' and this date was ' ||  UPPER(v_tmp_date)
       --  );
        IQ_EOM_DEV.G5_PICK_FEES_F(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      ELSIf UPPER(v_query_result2) != UPPER(sCust_start) 
      AND UPPER(v_query_result2) IS NOT NULL 
      AND UPPER(v_query_logfile) != UPPER(v_tmp_date) Then
      ...
      END IF;
      nCheckpoint := 9;
      IQ_EOM_DEV.I_EOM_MISC_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);

      nCheckpoint := 10;
      IQ_EOM_DEV.K1_PAL_DESP_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      nCheckpoint := 11;
      IQ_EOM_DEV.K2_CTN_IN_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      nCheckpoint := 12;
      IQ_EOM_DEV.K3_PAL_IN_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      nCheckpoint := 13;
      IQ_EOM_DEV.K4_CTN_DESP_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);


     If ( sCust_start = 'VHAAUS' ) Then
        nCheckpoint := 14;
        IQ_EOM_DEV.J_EOM_CUSTOMER_FEES_VHA(p_array_size_start,start_date,end_date,sCust_start);
      ElsIf ( sCust_start = 'BEYONDBLUE' ) Then
        nCheckpoint := 15;
        IQ_EOM_DEV.J_EOM_CUSTOMER_FEES_BB(p_array_size_start,start_date,end_date,sCust_start);
      ElsIf ( sCust_start = 'TABCORP' ) Then
        nCheckpoint := 16;
        IQ_EOM_DEV.J_EOM_CUSTOMER_FEES_TAB(p_array_size_start,start_date,end_date,sCust_start);
      --ElsIf ( sCust_start = 'IAG' ) Then
        --nCheckpoint := 60;
        --IQ_EOM_DEV.Z_EOM_RUN_IAG(p_array_size_start,start_date,end_date,'CGU',sAnalysis_Start);
      End If;
     ...
     EXECUTE IMMEDIATE v_query;
     COMMIT;
     Y_EOM_DEV_MERGE_ALL_FEES2();
     
      nCheckpoint := 100;
     v_query := 'TRUNCATE TABLE DEV_ALL_FEES_F';
     EXECUTE IMMEDIATE v_query;
     COMMIT;
     Y_EOM_DEV_MERGE_ALL_FEES_FINAL(sCust_start);

     nCheckpoint := 101;
      ----DBMS_OUTPUT.PUT_LINE('START Z DEV_ALL_FEES for ' || sFileName|| ' saved in ' || sPath );
      Z1_DEV_ALL_FEES_TO_CSV(sFileName);

    v_query2 :=  SQL%ROWCOUNT;
    -- --DBMS_OUTPUT.PUT_LINE('Z EOM Successfully Ran EOM_RUN_ALL for ' || sCust_start|| ' in ' ||(round((dbms_utility.get_time-l_start)/100, 2) ||
    --' Seconds...' );
      v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
      IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Z_EOM_RUN_ALL','MERGE','NULL',v_time_taken,SYSTIMESTAMP,sCust_start);
  ...
  END Z3_EOM_RUN_ALL;
    ...
    BEGIN
      v_query  := q'{Select /*+INDEX(DEV_EOM_LOGS LAST_TOUCHED)*/ TO_DATE From DEV_EOM_LOGS Where DEST_TBL = :v_in_tbl AND ROWNUM <= 1   }';
      --,gds_src_get_desp_stocks OUT sys_refcursor
   ...
   END EOM_INSERT_LOG;    
    ...
    BEGIN
      ...
      --need to allow for customer based query changes as well as last date range
          Select /*+INDEX(DEV_EOM_LOGS LAST_TOUCHED)*/ TO_DATE
          INTO  v_rtn_rslt
          FROM DEV_EOM_LOGS Where DEST_TBL = v_in_tbl AND ROWNUM <= 1;
          RETURN v_rtn_rslt;
    END F_EOM_CHECK_LOG;
    ...
    BEGIN
      ----DBMS_OUTPUT.PUT_LINE(' No Table name has been entered, nothing to return??? table was ' || v_in_tbl ); 
      --need to allow for customer based query changes as well as last date range
          Select /*+INDEX(DEV_EOM_LOGS LAST_TOUCHED)*/ CUST --using this field to populate from table with customer code from query
          INTO  v_rtn_rslt
          FROM DEV_EOM_LOGS Where DEST_TBL = v_in_tbl AND ROWNUM <= 1;
          RETURN v_rtn_rslt;
    END F_EOM_CHECK_CUST_LOG;
    
    PROCEDURE Z1_DEV_ALL_FEES_TO_CSV( p_filename in varchar2 )
    is
        ...
        l_query         varchar2(1000)
                       default 'select * from DEV_ALL_FEES_F';
       l_colCnt        number := 0;
       l_separator     varchar2(1);
       l_descTbl       dbms_sql.desc_tab;
       v_time_taken VARCHAR2(205);
        sPath VARCHAR2(60) :=  'EOM_ADMIN_DEV';
        l_start number default dbms_utility.get_time;
   begin
       l_output := utl_file.fopen( 'EOM_ADMIN_DEV', p_filename, 'w' );
      ...
      -- v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
       --IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Z1_DEV_ALL_FEES_TO_CSV','CSV','DEV_ALL_FEES_F',v_time_taken,SYSTIMESTAMP,sCustomerCode);
     
       --DBMS_OUTPUT.PUT_LINE('Z DEV_ALL_FEES for ' || p_filename || ' saved in ' || sPath );
    exception
       when others then
           execute immediate 'alter session set nls_date_format=''dd-MON-yy'' ';
      raise;
   end Z1_DEV_ALL_FEES_TO_CSV;
    
    PROCEDURE Z2_DEV_FEES_TO_CSV( p_filename in varchar2, p_in_table in varchar2 )
    is
        l_output        utl_file.file_type;
        l_theCursor     integer default dbms_sql.open_cursor;
        l_columnValue   varchar2(4000);
   ...
   begin
       ...
       execute immediate 'alter session set nls_date_format=''dd-MON-yy'' ';
       --v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
       --IQ_EOM_DEV.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Z2_DEV_FEES_TO_CSV','CSV',p_in_table,v_time_taken,SYSTIMESTAMP,sCustomerCode);
     
       --DBMS_OUTPUT.PUT_LINE('Z2_DEV_FEES_TO_CSV for ' || p_filename || ' saved in ' || sPath || ', data was from ' || p_in_table );
    exception
       when others then
           execute immediate 'alter session set nls_date_format=''dd-MON-yy'' ';
      raise;
   END Z2_DEV_FEES_TO_CSV;
   
     PROCEDURE get_stockonhand_curp (
          gsc_cust_in IN Dev_Group_Cust.sGroupCust%TYPE,
    ...
    BEGIN
      ...
      FROM  NA n INNER JOIN IL l ON l.IL_UID = n.NA_EXT_KEY
            ...
            --LEFT JOIN Dev_Group_Cust r ON r.sGroupCust = i.IM_CUST
...
BEGIN
  ...
  INNER JOIN Dev_Group_Cust r ON r.sGroupCust = IM_CUST
  ...
  BEGIN
     ...
     -- --DBMS_OUTPUT.PUT_LINE(l_data(i).Customer || ' - ' || l_data(i).Parent || '.' );
      --CLOSE EOM_CUSTS;
       ...
       --   --DBMS_OUTPUT.PUT_LINE(l_data(i).Customer || ' - ' || l_data(i).Parent || '.' );
        --END LOOP;
     FOR rec IN (SELECT RM_CUST FROM RM WHERE RM_XX_EOM_ADMIN = 'ADMIN' AND RM_CUST NOT IN (Select CUST FROM DEV_EOM_LOGS WHERE ORIGIN_PROCESS = 'Z_EOM_RUN_ALL' AND SubStr(LAST_SUC_FIN,0,9) = check_date ))
