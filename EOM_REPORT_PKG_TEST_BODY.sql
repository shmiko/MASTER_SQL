...
END C_EOM_START_CUST_TEMP_DATA;   
 ...
 SELECT To_Number(regexp_substr(RM_XX_FEE13,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO nRM_XX_FEE13 FROM RM where RM_CUST = sCust;
  ...
  PROCEDURE EOM_TMP_AUTO_FREIGHT_ALL (
	...
	--AND       (r.sGroupCust = sCust2 OR r.sCust = sCust2)
...
--USING ;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('EOM Temp Freight for all customer for the date range ' 
      || start_date || ' -- ' || end_date);
      RAISE;

  END EOM_TMP_AUTO_FREIGHT_ALL;
...
/*Manual freight fees*/
	 select    s.SH_CUST                AS "Customer",
      ...
      --	AND       r.RM_ANAL NOT IN ('21VICP','22NSWP')-- :sAnalysis
        --AND       (r.sGroupCust = :cust OR r.sCust = :cust)
...
--USING ;
  ...
  COMMIT;
  --RETURN;
  DBMS_OUTPUT.PUT_LINE('EOM Temp Freight for all customers for the date range ' 
      || start_date || ' -- ' || end_date);
    --COMMIT;
  EXCEPTION
    WHEN OTHERS THEN
  ...
  PROCEDURE EOM_TMP_ALL_HAND_FEES_ALL_CUST (
   ...
   /*ShrinkWrap Fee*/
	SELECT    s.SH_CUST,
        ...
        ELSE ''
        END                     AS "UOI",
        CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN  (Select To_Number(RM_XX_FEE18) from RM where RM_CUST = sCust)
        ELSE null
        END                      AS "UnitPrice",
        CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN  (Select To_Number(RM_XX_FEE18) from RM where RM_CUST = sCust)
        ELSE null
        ...
        ELSE NULL
        END                                            AS "Excl_Total",
        CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN  (Select To_Number(RM_XX_FEE18) from RM where RM_CUST = sCust)  * 1.1
        ELSE NULL
        END                                           AS "DIncl",
        CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN  (Select To_Number(RM_XX_FEE18) from RM where RM_CUST = sCust)  * 1.1
        ELSE NULL
    ...
    /*Handeling Fee*/
	  SELECT    s.SH_CUST                AS "Customer",
	    ...
	    CASE    WHEN t.vSLPslip IS NOT NULL THEN  '1'
			    ELSE ''
			    END                     AS "UOI",
	    CASE    WHEN t.vSLPslip IS NOT NULL THEN  (Select To_Number(RM_XX_FEE06) from RM where RM_CUST = sCust)
			    ELSE NULL
			    END                      AS "UnitPrice",
	    CASE    WHEN t.vSLPslip IS NOT NULL THEN  (Select To_Number(RM_XX_FEE06) from RM where RM_CUST = sCust)
			    ELSE NULL
			  ...
			  CASE    WHEN t.vSLPslip IS NOT NULL THEN  (Select To_Number(RM_XX_FEE06) from RM where RM_CUST = sCust)
			    ELSE NULL
			    END                                 AS "Excl_Total",
	    CASE    WHEN t.vSLPslip IS NOT NULL THEN (Select To_Number(RM_XX_FEE06) from RM where RM_CUST = sCust) * 1.1
			    ELSE NULL
			    END                      AS "DIncl",
	    CASE    WHEN t.vSLPslip IS NOT NULL THEN   (Select To_Number(RM_XX_FEE06) from RM where RM_CUST = sCust)  * 1.1
			    ELSE NULL
...
/*DB Maintenance Fee*/
	SELECT    RM_CUST                AS "Customer",
	  ...
	  '1'           AS "UOI",
	   TO_NUMBER(RM_XX_FEE21)          AS "UnitPrice",
	   TO_NUMBER(RM_XX_FEE21)                    AS "OWUnitPrice",
	   TO_NUMBER(RM_XX_FEE21) * (SELECT Count(RD_CODE) FROM PWIN175.RD WHERE RD_CUST IN ( SELECT RM_CUST FROM PWIN175.RM WHERE (RM_PARENT = sCust) AND SubStr(RD_CODE,0,2) NOT LIKE 'WH'  AND RD_CODE <> 'DIRECT'))         AS "DExcl",
	   TO_NUMBER(RM_XX_FEE21) * (SELECT Count(RD_CODE) FROM PWIN175.RD WHERE RD_CUST IN ( SELECT RM_CUST FROM PWIN175.RM WHERE (RM_PARENT = sCust) AND SubStr(RD_CODE,0,2) NOT LIKE 'WH'  AND RD_CODE <> 'DIRECT'))      AS "Excl_Total",
		( TO_NUMBER(RM_XX_FEE21) * (SELECT Count(RD_CODE) FROM PWIN175.RD WHERE RD_CUST IN ( SELECT RM_CUST FROM PWIN175.RM WHERE (RM_PARENT = sCust) AND SubStr(RD_CODE,0,2) NOT LIKE 'WH'  AND RD_CODE <> 'DIRECT'))) * 1.1         AS "DIncl",
	  ( TO_NUMBER(RM_XX_FEE21) * (SELECT Count(RD_CODE) FROM PWIN175.RD WHERE RD_CUST IN ( SELECT RM_CUST FROM PWIN175.RM WHERE (RM_PARENT = sCust) AND SubStr(RD_CODE,0,2) NOT LIKE 'WH'  AND RD_CODE <> 'DIRECT'))) * 1.1        AS "Incl_Total",
		 TO_NUMBER(RM_XX_FEE21)                     AS "ReportingPrice",
			  NULL                     AS "Address",
			  NULL                     AS "Address2",
			  NULL                     AS "Suburb",
			  NULL                     AS "State",
...
/*Stock Maint Charges including Stocktake and Kitting*/
    SELECT    RM_CUST                AS "Customer",
	  ...
	  '1'           AS "UOI",
	   TO_NUMBER(RM_XX_FEE20)          AS "UnitPrice",
	   TO_NUMBER(RM_XX_FEE20)                   AS "OWUnitPrice",
	   TO_NUMBER(RM_XX_FEE20) * total_count_by_cust(sCust)         AS "DExcl",
	   TO_NUMBER(RM_XX_FEE20) * total_count_by_cust(sCust)      AS "Excl_Total",
		( TO_NUMBER(RM_XX_FEE20) * total_count_by_cust(sCust)) * 1.1         AS "DIncl",
	  ( TO_NUMBER(RM_XX_FEE20) * total_count_by_cust(sCust)) * 1.1        AS "Incl_Total",
		 TO_NUMBER(RM_XX_FEE20)                    AS "ReportingPrice",
			  NULL                     AS "Address",
			  NULL                     AS "Address2",
			  NULL                     AS "Suburb",
			  NULL                     AS "State",
	...
	FROM  PWIN175.RM
	WHERE    TO_NUMBER(RM_XX_FEE20) > 0
	AND     (RM_CUST = sCust)
  GROUP BY  RM_CUST,RM_XX_FEE20
...
/*Admin Charges*/
	  ...
	  CASE    WHEN RM_CUST IS NOT NULL THEN 'Admin fee '
       ...
       CASE    WHEN RM_CUST IS NOT NULL THEN 1
			  ...
			  END                      AS "Qty",
	   '1'           AS "UOI",
	      TO_NUMBER(RM_XX_FEE19)  AS "UnitPrice",
	   TO_NUMBER(RM_XX_FEE19)  AS "OWUnitPrice",
	   TO_NUMBER(RM_XX_FEE19)  AS "DExcl",
	   TO_NUMBER(RM_XX_FEE19)  AS "Excl_Total",
		( TO_NUMBER(RM_XX_FEE19)  * 1.1)         AS "DIncl",
	  ( TO_NUMBER(RM_XX_FEE19) * 1.1)        AS "Incl_Total",
		 TO_NUMBER(RM_XX_FEE19)                    AS "ReportingPrice",
			  NULL                     AS "Address",
			  NULL                     AS "Address2",
			  NULL                     AS "Suburb",
			  NULL                     AS "State",
	...
	FROM  PWIN175.RM
	WHERE     TO_NUMBER(RM_XX_FEE19) > 0
	AND     RM_CUST = sCust
 GROUP BY  RM_CUST,RM_XX_FEE19
...
/*Carton Despatch Fee*/
	SELECT    s.SH_CUST                AS "Customer",
	  ...
	  CASE    WHEN t.ST_XX_NUM_CARTONS >= 1 THEN  '1'
			  ELSE ''
			  END                     AS "UOI",
	  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1 THEN (SELECT To_Number(regexp_substr(RM_XX_FEE15,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCust) --  (Select To_Number(RM_XX_FEE15) from RM where RM_CUST = sCust)
			 ELSE null
			 END                      AS "UnitPrice",
	  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1 THEN (SELECT To_Number(regexp_substr(RM_XX_FEE15,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCust) --  (Select To_Number(RM_XX_FEE15) from RM where RM_CUST = sCust)
			 ELSE null
	 ...
	 CASE   WHEN t.ST_XX_NUM_CARTONS >= 1  THEN (SELECT To_Number(regexp_substr(RM_XX_FEE15,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCust) -- (Select To_Number(RM_XX_FEE15) from RM where RM_CUST = sCust)
			 ...
			 ELSE NULL
			 END                                            AS "Excl_Total",
	  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1 THEN (SELECT To_Number(regexp_substr(RM_XX_FEE15,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCust) * 1.1-- (Select To_Number(RM_XX_FEE15) from RM where RM_CUST = sCust)  * 1.1
			 ELSE NULL
			 END                                           AS "DIncl",
	  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1 THEN (SELECT To_Number(regexp_substr(RM_XX_FEE15,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCust) * 1.1 -- (Select To_Number(RM_XX_FEE15) from RM where RM_CUST = sCust)  * 1.1
			 ELSE NULL
	...
	FROM      PWIN175.SD d
			  ...
			  LEFT JOIN Tmp_Group_Cust r ON r.sCust = s.SH_CUST
			  INNER JOIN PWIN175.IM i  ON i.IM_STOCK = d.SD_STOCK
	WHERE  (SELECT To_Number(regexp_substr(RM_XX_FEE15,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCust) > 0
	AND       s.SH_STATUS <> 3
	AND       (r.sGroupCust = sCust OR r.sCust = sCust)
	AND       s.SH_ORDER = t.ST_ORDER
