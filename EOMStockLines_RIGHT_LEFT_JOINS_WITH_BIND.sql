...
exec :start_date := To_Date('30-Apr-2014')
var end_date varchar2(20)
exec :end_date := To_Date('30-Apr-2014')
...
/*Stocks*/
	...
	SELECT    s.SH_CUST                AS "Customer",
      ...
      CASE  WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN d.SD_SELL_PRICE * d.SD_QTY_DESP
            ...
            WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NOT NULL THEN  eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) * d.SD_QTY_DESP
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NULL THEN  d.SD_XX_OW_UNIT_PRICE * d.SD_QTY_DESP
			      ELSE NULL
			      END          AS "DExcl",
	  ...
	  CASE    WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN (d.SD_SELL_PRICE * d.SD_QTY_DESP) * 1.1
            ...
            WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NOT NULL THEN  (eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) * d.SD_QTY_DESP) * 1.1
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST = :cust2 AND eom_report_pkg.F_BREAK_UNIT_PRICE(r.RM_GROUP_CUST,d.SD_STOCK) IS NULL THEN  (d.SD_XX_OW_UNIT_PRICE * d.SD_QTY_DESP) * 1.1
			      ELSE NULL
			      END          AS "DIncl",
	   CASE   WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 0 THEN (d.SD_SELL_PRICE * d.SD_QTY_DESP) * 1.1
			      WHEN d.SD_STOCK IS NOT NULL AND i.IM_CUST <> :cust2 AND i.IM_OWNED_BY = 1 THEN  ((n.NI_SELL_VALUE/n.NI_NX_QUANTITY) * d.SD_QTY_DESP) * 1.1
