...
exec :start_date := To_Date('20-Jul-2013')
var end_date varchar2(20)
exec :end_date := To_Date('22-Jul-2013')
...
SELECT    s.SH_CUST                AS "Customer",
  ...
  CASE    WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN 'ShrinkWrap Fee is '
          ELSE ''
          END                      AS "FeeType",
          d.SD_STOCK               AS "Item",
  ...
  CASE    /* Get Packing Fees If stock is of type BB_PACK then charge sRM_XX_FEE08.AsDouble * SL_PSLIP_QTY  */
	       WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN '' ||  (Select RM_XX_FEE18 from RM where RM_CUST = :cust )
         ELSE ''
         END                      AS "UnitPrice",
         d.SD_EXCL                AS "DExcl",
