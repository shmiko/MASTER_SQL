CREATE OR REPLACE PACKAGE EOM_REPORT_PKG_PROD
IS
  ...
  RETURN NUMBER;
    ...
    PROCEDURE GET_IAGRACV_PDS_DEBUG    (
     ...
     PROCEDURE OWUSER4(c_test IN OUT sys_refcursor);                             
    ...
    PROCEDURE get_desp_stocks_curp 
      (
			...
			--,gds_nx_ext_type_in IN NI.NI_NV_EXT_TYPE%TYPE
			 ,gds_stock_in IN SD.SD_STOCK%TYPE
			,gds_stock_not_in2 IN SD.SD_STOCK%TYPE
			,gds_start_date_in IN SH.SH_EDIT_DATE%TYPE
			,gds_end_date_in IN SH.SH_ADD_DATE%TYPE
			,gds_src_get_desp_stocks OUT sys_refcursor
      );
      
      FUNCTION F_ORDER_COUNT
      RETURN NUMBER;
    
    PROCEDURE get_daily_order_curp (
			gds_src_get_ord_cnt OUT sys_refcursor
    ); 
    
    PROCEDURE get_daily_order_curp2 (
			gds_src_get_ord_cnt OUT sys_refcursor
    );
    
    PROCEDURE get_daily_desp_curp (
			gds_src_get_desp_cnt OUT sys_refcursor
    );
    
     PROCEDURE get_daily_pick_curp (
			gds_src_get_pick_cnt OUT sys_refcursor
    );
    
    PROCEDURE get_pick_lines_curp (
      gds_SL_PSLIP_in2 IN SL.SL_PSLIP%TYPE,
			gds_SL_EDIT_DATE_start_in IN SL.SL_EDIT_DATE%TYPE,
			gds_src_get_pick_cnt OUT sys_refcursor
    );
    
    
    PROCEDURE get_daily_rec_curp (
			gds_src_get_recs_cnt OUT sys_refcursor
    );
  ...
  RETURN NUMBER;
   ...
   FUNCTION f_GetWarehouse_from_SD
    ...
    FUNCTION f_GetCustFromAnal
		(
		  analysis IN RM.RM_ANAL%TYPE
		  )
    RETURN VARCHAR2;
    
    FUNCTION f_Get_Charge_from_SD_IC
		(
		  analysis IN RM.RM_ANAL%TYPE
      ,v_type_in NUMBER
		  )
   ...
   PROCEDURE EOM_CREATE_TEMP_LOG_DATA
      ...
      PROCEDURE EOM_CREATE_TEMP_LOG_DATA_VALUE
        (
        start_date IN SH.SH_ADD_DATE%TYPE
        ,end_date  IN SH.SH_ADD_DATE%TYPE
        ,cust       IN VARCHAR2
...
END EOM_REPORT_PKG_PROD;
/


CREATE OR REPLACE PACKAGE BODY EOM_REPORT_PKG_PROD
AS
 ...
 --TYPE t_custtype AS TABLE OF custtype;
  ...
  IS
   CURSOR soh_cur    IS
      SELECT COUNT(e.NE_AVAIL_ACTUAL)
      ...
      WHERE l.IL_LOCN = l_locn_in
      AND (e.NE_STATUS = 1 OR e.NE_STATUS = 2)
      GROUP BY l.IL_LOCN;
...
ORDER BY 6,1,3 Asc
 ...
 END GET_IAGRACV_PDS;
  ...
  SELECT   'TOTALS' AS "SCAN"
           ...
           END AS "Status" 
          ,F_GET_PDS_COUNTS(SH_ORDER) As "PDS Count"
          ,CASE WHEN SD1.SD_ADD_OP = 'SERV1' THEN 
                  F_GET_PDS_COUNTS2(SH_ORDER,'T')
            ELSE
                  F_GET_PDS_COUNTS2(SH_ORDER,'F')
            END As "SPDS Count"
          ,F_GET_PDS_SUMS(SD1.SD_ORDER,SD1.SD_STATUS) As "PDS EA QTY"
          ,CASE WHEN SD1.SD_ADD_OP = 'SERV1' THEN
                  F_GET_PDS_SUMS2(SD1.SD_ORDER,SD1.SD_STATUS,'T') 
          Else 
                  F_GET_PDS_SUMS2(SD1.SD_ORDER,SD1.SD_STATUS,'F')
          END As "SPDS EA QTY"
          ,CASE WHEN SD1.SD_ADD_OP = 'SERV1' 
                  THEN  CASE 
                            WHEN F_GET_PDS_SUMS(SD1.SD_ORDER,SD1.SD_STATUS) = F_GET_PDS_SUMS2(SD1.SD_ORDER,SD1.SD_STATUS,'T') 
                            THEN 'PASS'
                            ELSE 'FAIL'
                        END 
                  ELSE  CASE 
                            WHEN F_GET_PDS_SUMS(SD1.SD_ORDER,SD1.SD_STATUS) = F_GET_PDS_SUMS2(SD1.SD_ORDER,SD1.SD_STATUS,'F') 
                            THEN 'PASS'
                            ELSE 'FAIL'
                        END 
          END As "PASS-FAIL"
          FROM SH
          LEFT JOIN SD SD1 ON SD1.SD_ORDER = SH_ORDER --INNER JOIN SD SD2 ON SD2.SD_LINE = SD1.SD_LINE
          INNER JOIN IM ON IM.IM_STOCK = SD1.SD_STOCK
        WHERE SD1.SD_ADD_DATE >= iagracv_date_from_in AND SD1.SD_ADD_DATE <= iagracv_date_to_in
        AND IM_CUST IN ('RACV','IAG')       --Just RACV or IAG
        AND SD1.SD_STATUS <> 3              --No cancelled lines
        AND F_GET_PDS_COUNTS(SH_ORDER) <> 0 --Must have at least 1 PDS Stock
        --AND SH_ORDER = '   1585603'         --Use a single order number for testing due to buffer output
       ORDER BY 6,1,3 Asc;
  iagracv_rec iagracv_cur%ROWTYPE;
  BEGIN
    OPEN iagracv_cur;
    ...
    WHILE iagracv_cur%FOUND
    LOOP
    DBMS_OUTPUT.PUT_LINE(iagracv_rec.SH_ORDER || ',' || iagracv_rec.SH_CUST || ',' || iagracv_rec.SH_ADD_DATE || ',' || iagracv_rec.SH_REQ_DEL_DATE );
      FETCH iagracv_cur INTO iagracv_rec;
    END LOOP;
  CLOSE iagracv_cur;
 END GET_IAGRACV_PDS_DEBUG;
    ...
    PROCEDURE OWUSER4(c_test IN OUT sys_refcursor) AS 
    BEGIN
      open c_test for select vm_profile, vm_name from vm where vm_profile LIKE '19%';
    END OWUSER4;
  ...
  v_query           CLOB;
     ...
     v_query := q'{  SELECT 'PDS' AS "SCAN"
          ...
          ,NULL  AS "PASS/FAIL"
          ,NULL  AS "OriginalPromoQty"
          ,F_GET_PDS_COUNTS2(SD.SD_ORDER,:start_date,:end_date,'IA_ALT_STOCK')  As "SPDS_Count"      
          ,F_GET_PDS_COUNTS(SD.SD_ORDER,:start_date,:end_date,'IA_STOCK')  As "PDS_Count"   
          ,NULL AS "IA_ALT_STOCK"
          ,NULL AS "IA_STOCK"
          FROM SD
          INNER JOIN IM ON IM.IM_STOCK = SD.SD_STOCK
  ...
  --BEGIN
   ...
   -- LOOP
    OPEN iagracv_cur FOR v_query
     USING   iagracv_date_to_in,iagracv_date_from_in, iagracv_date_to_in, iagracv_date_from_in, iagracv_date_to_in, iagracv_date_from_in, iagracv_date_to_in, iagracv_date_from_in, iagracv_date_to_in, iagracv_date_from_in; 
     DBMS_OUTPUT.PUT_LINE('Successfully ran GET_IAGRACV_PDS2');
 ...
 END GET_IAGRACV_PDS2;
  ...
  v_query           CLOB;
	 BEGIN
     v_query := q'{         SELECT DISTINCT SH_ORDER, 'TOTALS' AS "SCAN"
           ,SH_ADD_DATE
           ,SH_REQ_DEL_DATE
           ,SH_ORDER
           ,SH_ORDER
           , CASE
              ...
              WHEN SH_STATUS = 4 THEN 'Closed'
           END AS "Status" 
          ,F_GET_PDS_COUNTS(SH_ORDER)
          ,CASE WHEN SD1.SD_ADD_OP = 'SERV1' THEN 
                  F_GET_PDS_COUNTS2(SH_ORDER,'T')
          ELSE
                  F_GET_PDS_COUNTS2(SH_ORDER,'F')
          END AS "SPDS Count"
          ,F_GET_PDS_SUMS(SD1.SD_ORDER,SD1.SD_STATUS)
          ,CASE WHEN SD1.SD_ADD_OP = 'SERV1' THEN
                  F_GET_PDS_SUMS2(SD1.SD_ORDER,SD1.SD_STATUS,'T') 
          Else 
                  F_GET_PDS_SUMS2(SD1.SD_ORDER,SD1.SD_STATUS,'F')
    ...
    OPEN iagracv_cur FOR v_query
    USING   iagracv_date_to_in,iagracv_date_from_in;
    DBMS_OUTPUT.PUT_LINE('Successfully ran GET_IAGRACV_PDS3');

 END GET_IAGRACV_PDS3;
...
BEGIN
  ...
  SELECT     SH.SH_CUST   AS "Customer"
           ...
           ,SD.SD_XX_PICKLIST_NUM     AS "XX PickNum"
 ...
 END get_desp_stocks_curp; 
  ...
  BEGIN
    nbreakpoint := 1;
        ...
        INTO  order_count	     
        FROM SH S 
        WHERE S.SH_ADD_DATE = TO_CHAR (SYSDATE);
        
        RETURN order_count;
  EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('get_daily_order_curp failed at checkpoint ' || nbreakpoint ||
                          ' with error ' || SQLCODE || ' : ' || SQLERRM);
      RAISE;  
  END F_ORDER_COUNT; 
...
BEGIN
  nbreakpoint := 1;
  OPEN gds_src_get_ord_cnt FOR
  SELECT Count(SH_ORDER)
      FROM SH LEFT JOIN Tmp_Group_Cust r ON r.sCust = sh.SH_CUST
      WHERE to_char(SH.SH_ADD_DATE ,'dd/mm/yyyy') = ( SELECT to_char(sysdate,'dd/mm/yyyy') FROM dual);
      DBMS_OUTPUT.PUT_LINE('get_daily_order_curp succeded ');
 EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('get_daily_order_curp failed at checkpoint ' || nbreakpoint ||
                          ' with error ' || SQLCODE || ' : ' || SQLERRM);
      RAISE;
 END get_daily_order_curp; 
...
BEGIN
  ...
  SELECT Count(SH_ORDER)
      FROM SH S 
        WHERE S.SH_ADD_DATE = TO_CHAR (SYSDATE)
        AND S.SH_CAMPAIGN NOT IN( 'ADMIN','OBSOLETE');
      DBMS_OUTPUT.PUT_LINE('get_daily_order_curp succeded ');
 EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('get_daily_order_curp failed at checkpoint ' || nbreakpoint ||
...
BEGIN
  nbreakpoint := 1;
  OPEN gds_src_get_desp_cnt FOR
  SELECT Count(ST_ORDER)
       FROM ST
       WHERE ST.st_desp_date = TO_CHAR (SYSDATE);
      DBMS_OUTPUT.PUT_LINE('get_daily_desp_curp succeded ');
 EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('get_daily_desp_curp failed at checkpoint ' || nbreakpoint ||
                          ' with error ' || SQLCODE || ' : ' || SQLERRM);
      RAISE;
 END get_daily_desp_curp; 
...
BEGIN
  nbreakpoint := 1;
  OPEN gds_src_get_recs_cnt FOR
  SELECT Count(NE_ENTRY)
  FROM  NA n INNER JOIN IL l ON l.IL_UID = n.NA_EXT_KEY
	INNER JOIN NE e ON e.NE_ACCOUNT = n.NA_ACCOUNT
	--INNER JOIN IM i ON i.IM_STOCK = n.NA_STOCK
  WHERE n.NA_EXT_TYPE = 1210067
	...
	AND e.NE_DATE = TO_CHAR (SYSDATE);
      
      DBMS_OUTPUT.PUT_LINE('get_daily_rec_curp succeded ');
 EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('get_daily_rec_curp failed at checkpoint ' || nbreakpoint ||
                          ' with error ' || SQLCODE || ' : ' || SQLERRM);
      RAISE;
 END get_daily_rec_curp; 
...
BEGIN
  nbreakpoint := 1;
  OPEN gds_src_get_pick_cnt FOR
 SELECT Count(SL_LINE)
      ...
      WHERE s.SL_PSLIP IS NOT NULL
      AND h.SH_CAMPAIGN NOT IN( 'ADMIN','OBSOLETE')
      --AND r2.RM_ACTIVE = 1   --This was the problem
      --AND r.sGroupCust LIKE '%'
      AND s.SL_EDIT_DATE = TO_CHAR (SYSDATE);
      DBMS_OUTPUT.PUT_LINE('get_daily_pick_curp succeded ');
 EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('get_daily_pick_curp failed at checkpoint ' || nbreakpoint ||
                          ' with error ' || SQLCODE || ' : ' || SQLERRM);
      RAISE;
 END get_daily_pick_curp; 
...
BEGIN
 ...
 SELECT Count(SL_LINE)
      ...
      --INNER JOIN RM r2 ON r2.RM_CUST = h.SH_CUST
      --INNER JOIN ST t ON t.ST_PICK = s.SL_PICK
      WHERE s.SL_PSLIP = gds_SL_PSLIP_in2
      AND h.SH_CAMPAIGN NOT IN( 'ADMIN','OBSOLETE')
      --AND r2.RM_ACTIVE = 1   --This was the problem
      --AND r.sGroupCust LIKE '%'
      AND s.SL_EDIT_DATE = gds_SL_EDIT_DATE_start_in;
      DBMS_OUTPUT.PUT_LINE('get_daily_pick_curp succeded ');
 EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('get_daily_pick_curp failed at checkpoint ' || nbreakpoint ||
 ...
 FUNCTION total_soh_by_stock
  ...
  IS
   CURSOR soh_cur    IS
      ...
      FROM NI 
      WHERE NI_STOCK = gsc_stock_in
      AND NI_AVAIL_ACTUAL > 0
      AND NI_STATUS IN ( 1,2)
      GROUP BY NI_STOCK;
...
BEGIN
 ...
 FUNCTION f_Get_Charge_from_SD
  ...
  AS
       v_rtn_value NUMBER;
  BEGIN
		  IF      v_type_in = 1 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE27,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = rm_cust_in; RETURN v_rtn_value;
       
		  ELSIF 	v_type_in = 2 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE06,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = rm_cust_in; RETURN v_rtn_value;
       
		  ELSIF  	v_type_in = 3 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE16,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = rm_cust_in; RETURN v_rtn_value;
       
		  ELSIF  	v_type_in = 4 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE14,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = rm_cust_in; RETURN v_rtn_value;
       
		  ELSIF  	v_type_in = 5 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE11,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = rm_cust_in; RETURN v_rtn_value;
       
      ELSIF  	v_type_in = 6 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE12,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = rm_cust_in; RETURN v_rtn_value;
       
		  END IF;
	  END f_Get_Charge_from_SD;
  ...
  IS
      v_rtn_value VARCHAR2(50);    
  BEGIN
		  IF analysis = '21VICP' Then
        v_rtn_value := 'CONNECTVIC'; 
      ELSIF analysis = '22NSWP' Then
        v_rtn_value := 'LINK';
      End IF;
  RETURN v_rtn_value;   
  END f_GetCustFromAnal;
  ...
  BEGIN
		  ...
		  IF      v_type_in = 1 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE27,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = EOM_REPORT_PKG_PROD.f_GetCustFromAnal(analysis); RETURN v_rtn_value;
       
		  ELSIF 	v_type_in = 2 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE06,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = EOM_REPORT_PKG_PROD.f_GetCustFromAnal(analysis); RETURN v_rtn_value;
       
		  ELSIF  	v_type_in = 3 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE16,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = EOM_REPORT_PKG_PROD.f_GetCustFromAnal(analysis); RETURN v_rtn_value;
       
		  ELSIF  	v_type_in = 4 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE14,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = EOM_REPORT_PKG_PROD.f_GetCustFromAnal(analysis); RETURN v_rtn_value;
       
		  ELSIF  	v_type_in = 5 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE11,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = EOM_REPORT_PKG_PROD.f_GetCustFromAnal(analysis); RETURN v_rtn_value;
       
      ELSIF  	v_type_in = 6 	THEN SELECT To_Number(regexp_substr(RM_XX_FEE12,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) INTO v_rtn_value FROM RM where RM_CUST = EOM_REPORT_PKG_PROD.f_GetCustFromAnal(analysis); RETURN v_rtn_value;
       
		  END IF;
	  END f_Get_Charge_from_SD_IC;
    
    
    
    
    
  FUNCTION f_GetPDS_Count_from_SD
		(
		  v_sd_locn_in VARCHAR2
		  )
    RETURN VARCHAR2
    ...
    /*Insert fresh temp data*/
        ...
        v_query := q'{
                      ...
                      SELECT  EOM_REPORT_PKG.f_GetWarehouse_from_SD(d.SD_LOCN) AS Warehouse,
                        sGroupCust As Cust,
                        Count(DISTINCT(SD_ORDER)) AS Total,
                        EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(sGroupCust,1) As Charge,
                        CASE WHEN is_number( EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(sGroupCust,1) ) = 'Y' Then
                          (Count(DISTINCT(SD_ORDER)) * EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(sGroupCust,1))
                        ELSE
                          null
                        END As TotalValue,
                        'A- Orders' AS ChType
                      FROM SH h LEFT JOIN Tmp_Group_Cust r ON r.sCust = h.SH_CUST
                        RIGHT JOIN SD d ON d.SD_ORDER = h.SH_ORDER
                        INNER JOIN RM r2 ON r2.RM_CUST = h.SH_CUST
                      WHERE h.SH_ADD_DATE >= :st_start_date AND h.SH_ADD_DATE <= :st_end_date
                        AND h.SH_STATUS <> 3
                        AND h.SH_CAMPAIGN NOT IN( 'ADMIN','OBSOLETE')
                        AND r2.RM_ACTIVE = 1   --This was the problem
                        AND r.sGroupCust LIKE '%'
                      GROUP BY ROLLUP 
                        (
                        EOM_REPORT_PKG.f_GetWarehouse_from_SD(d.SD_LOCN),
                        sGroupCust,
                        EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(sGroupCust,1)
                        )                           
                      HAVING EOM_REPORT_PKG.f_GetWarehouse_from_SD(d.SD_LOCN) LIKE '%'
            ...
            UNION ALL
                        ...
                        sGroupCust,
                        Count(*) AS Total,
                        EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(r2.RM_CUST,2) As Charge,
                        CASE WHEN is_number( EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(r2.RM_CUST,2) ) = 'Y' Then
                          (Count(*) * EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(r2.RM_CUST,2))
                        ELSE
                          null
                        END As TotalValue,
                        'B- Despatches' AS "Type"
                      ...
                      FROM  PWIN175.ST t INNER JOIN SL s ON s.SL_PICK = t.ST_PICK
                        ...
                        INNER JOIN RM r2 ON r2.RM_CUST = h.SH_CUST
                        --RIGHT JOIN SL s ON s.SL_PICK = t.ST_PICK                      
                      WHERE t.ST_DESP_DATE >= :st_start_date2 AND t.ST_DESP_DATE <= :st_end_date2
                        AND s.SL_LINE = 1
                        AND t.ST_PSLIP IS NOT NULL AND t.ST_PSLIP <> 'CANCELLED'
                        AND h.SH_STATUS <> 3
                        AND h.SH_CAMPAIGN NOT IN( 'ADMIN','OBSOLETE')
                      ...
                      GROUP BY ROLLUP 
                        (
                        EOM_REPORT_PKG.f_GetWarehouse_from_SD(s.SL_LOCN),
                        sGroupCust,
                        EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(r2.RM_CUST,2)
                        )     
                      HAVING   EOM_REPORT_PKG.f_GetWarehouse_from_SD(s.SL_LOCN) LIKE '%'


            UNION ALL
                    SELECT    EOM_REPORT_PKG.f_GetWarehouse_from_SD(s.SL_LOCN)  AS Warehouse,
                        sGroupCust AS Customer,
                        Count(s.SL_LINE) AS Total,
                        EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(r2.RM_CUST,3) As Charge,
                        CASE WHEN is_number( EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(r2.RM_CUST,3) ) = 'Y' Then
                          (Count(DISTINCT(s.SL_LINE)) * EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(r2.RM_CUST,3))
                        ELSE
                          null
                        END As TotalValue,
                        'C- Lines' AS "Type"
                      FROM  PWIN175.SL s INNER JOIN SH h ON s.SL_ORDER = h.SH_ORDER
                        ...
                        --INNER JOIN IL l ON l.IL_LOCN = s.SL_LOCN
                        INNER JOIN ST t ON t.ST_PICK = s.SL_PICK
                        WHERE s.SL_EDIT_DATE >= :st_start_date3 AND s.SL_EDIT_DATE <= :st_end_date3
                        AND s.SL_PSLIP IS NOT NULL
                        AND h.SH_CAMPAIGN NOT IN( 'ADMIN','OBSOLETE')
                        AND r2.RM_ACTIVE = 1   --This was the problem
                        AND r.sGroupCust LIKE '%'
                      GROUP BY ROLLUP 
                        ...
                        EOM_REPORT_PKG.f_GetWarehouse_from_SD(s.SL_LOCN),
                        sGroupCust,
                        EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(r2.RM_CUST,3)
                        ) 
                      HAVING EOM_REPORT_PKG.f_GetWarehouse_from_SD(s.SL_LOCN) LIKE '%'
           ...
           UNION ALL
                      ...
                      SELECT (CASE
                        ...
                        i.IM_CUST AS Cust,
                        Count(NE_ENTRY) AS Total,
                        EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,4) As Charge,
                        CASE WHEN is_number( EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,4) ) = 'Y' Then
                          (Count(DISTINCT(NE_ENTRY)) * EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,4))
                        ELSE
                          null
                        END As TotalValue,
                        'D- Receipts'  AS "Type"
                      ...
                      WHERE n.NA_EXT_TYPE = 1210067
                        ...
                        AND   e.NE_STRENGTH = 3
                        AND   (e.NE_STATUS = 1 OR e.NE_STATUS = 3)
                        AND   e.NE_DATE >= :st_start_date4 AND e.NE_DATE <= :st_end_date4
                        AND i.IM_CUST LIKE '%'
                      GROUP BY 
                        (
                          (CASE
                          ...
                          END),
                          i.IM_CUST,
                          EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,4)
                        )
                      HAVING  (CASE
                      WHEN l.IL_IN_LOCN IS NOT NULL THEN l.IL_IN_LOCN
                      WHEN l.IL_IN_LOCN IS NULL THEN l.IL_LOCN
         ...
         UNION ALL
                     SELECT
                        ...
                        END) AS Warehouse,
                        i.IM_CUST AS Cust,
                        total_count_by_locn(i.IM_CUST,l.IL_LOCN,l.IL_NOTE_2,l.IL_IN_LOCN) AS Total, 
                        Count(DISTINCT l.IL_LOCN) AS Total,  -- test a self join to rid the distinct
                        CASE WHEN Upper(substr(l.IL_NOTE_2,0,1)) = 'Y' THEN
                          EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,5)
                        ELSE 
                          EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,6)
                        END       As Charge,
                        CASE WHEN Upper(substr(l.IL_NOTE_2,0,1)) = 'Y' THEN
                          CASE WHEN is_number( EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,5) ) = 'Y' Then
                            (total_count_by_locn(i.IM_CUST,l.IL_LOCN,l.IL_NOTE_2,l.IL_IN_LOCN) * EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,5))
                          ELSE
                            null
                          END
                        ELSE 
                          CASE WHEN is_number( EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,6) ) = 'Y' Then
                            (total_count_by_locn(i.IM_CUST,l.IL_LOCN,l.IL_NOTE_2,l.IL_IN_LOCN) * EOM_REPORT_PKG_PROD.f_Get_Charge_from_SD(i.IM_CUST,6))
                          ELSE
                            null
                          END
                        END       As TotalValue,
           ...
           OPEN gds_src_get_desp_stocks FOR  v_query
          -- USING start_date, end_date, warehouse, start_date, end_date, warehouse, start_date, end_date, warehouse, start_date, end_date, warehouse;
           USING start_date, end_date, start_date, end_date,start_date, end_date, start_date, end_date;
           DBMS_OUTPUT.PUT_LINE('Successfully inserted fresh temporary data for pallets and shelves using NO warehouse and customer filters - should have passed NULL values and returned all data');
  ...
  EXCEPTION
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('EOM processing failed at checkpoint ' || nCheckpoint ||
                          ' with error ' || SQLCODE || ' : ' || SQLERRM);
      RAISE;
  END EOM_CREATE_TEMP_LOG_DATA_VALUE;
