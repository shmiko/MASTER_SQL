	...
	SELECT  s.SH_CUST                AS "Customer",
			...
			t.nCountOfLines           AS "Qty",
			 CASE    WHEN t.vSLPslip IS NOT NULL THEN  '1'
			  ELSE ''
			  END                     AS "UOI",
			 CASE    WHEN t.vSLPslip IS NOT NULL THEN :nRM_XX_FEE16 -- (Select To_Number(RM_XX_FEE16) from RM where RM_CUST = :cust)
			  ELSE NULL
			  END                      AS "UnitPrice",
		  CASE    WHEN t.vSLPslip IS NOT NULL THEN :nRM_XX_FEE16 --  (Select To_Number(RM_XX_FEE16) from RM where RM_CUST = :cust) * t.nCountOfLines
				  ELSE NULL
				  END                      AS "DExcl",
				CASE    WHEN t.vSLPslip IS NOT NULL THEN :nRM_XX_FEE16 -- (Select To_Number(RM_XX_FEE16) from RM where RM_CUST = :cust)
			  ELSE NULL
			  ...
			  ELSE NULL
			  END                                 AS "Excl_Total",
		  CASE    WHEN t.vSLPslip IS NOT NULL THEN :nRM_XX_FEE16 * 1.1 -- ((Select To_Number(RM_XX_FEE16) from RM where RM_CUST = :cust) * t.nCountOfLines) * 1.1
				  ELSE NULL
				  END                      AS "DIncl",
		  CASE    WHEN t.vSLPslip IS NOT NULL THEN :nRM_XX_FEE16 * 1.1 --  ((Select To_Number(RM_XX_FEE16) from RM where RM_CUST = :cust) * t.nCountOfLines)  * 1.1
				  ELSE NULL
