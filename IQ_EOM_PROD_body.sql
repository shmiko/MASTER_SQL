...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body IQ_EOM_PROD
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "PWIN175"."IQ_EOM_PROD" AS
   ...
   RETURN;
    ...
    BEGIN
     nCheckpoint := 15;
      ...
      If F_IS_TABLE_EEMPTY('Tmp_Locn_Cnt_By_Cust') <= 0 Then
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'C_EOM_START_ALL_TEMP_STOR_DATA','IL/NI','Tmp_Locn_Cnt_By_Cust',v_time_taken,SYSTIMESTAMP,NULL);
  ...
  --USING ;
    ...
    COMMIT;
      ...
      IF v_query2 > 0 THEN
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
        EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'F_EOM_TMP_VAN_FREIGHT_ALL','SD','TMP_V_FREIGHT',v_time_taken,SYSTIMESTAMP,sCustomerCode);
   ...
   -- sCust_Rates2 RM.RM_XX_FEE12%TYPE;
    ...
    EXECUTE IMMEDIATE v_query;
        ...
        --add another quick data check to ensure the source data is ok
          ...
          v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
          EOM_INSERT_LOG(SYSTIMESTAMP ,startdate,enddate,'H4_EOM_ALL_STOR','IL','TMP_STOR_FEES',v_time_taken,SYSTIMESTAMP,sCustomerCode);
...
*/
   ...
   (SELECT To_Number(regexp_substr(RM_XX_FEE15,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) FROM RM where RM_CUST = sCustomerCode) > 0
    ...
    AS
     ...
     OPEN c;
      ...
      COMMIT; 
        v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
          EOM_INSERT_LOG(SYSTIMESTAMP ,gds_start_date_in,gds_end_date_in,'L_DESPATCH_REPORT','TMP_DESP_REPT','ST',v_time_taken,SYSTIMESTAMP,'LINK');
          sFileName := 'LINK-L_DESPATCH_REPORT-' || gds_start_date_in || '-TO-' || gds_end_date_in || '-RunOn-' || sFileTime || '_A.csv';
          IQ_EOM_PROD.L_DESPATCH_REPORTB(p_array_size,gds_analysis,gds_next_start_date_in,gds_next_end_date_in);
    ...
    AS
     ...
     OPEN c;
      ...
      COMMIT; 
      v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
          EOM_INSERT_LOG(SYSTIMESTAMP ,gds_start_date_in,gds_end_date_in,'L_DESPATCH_REPORTB','TMP_DESP_REPT2','ST',v_time_taken,SYSTIMESTAMP,'LINK');
  ...
  BEGIN
   ...
   -- Else
    ...
    nCheckpoint := 5;
      ...
      --IQ_EOM_PROD.F_EOM_TMP_ALL_FREIGHT_ALL(p_array_size_start,start_date,end_date);
      --IQ_EOM_PROD.F8_Z_EOM_RUN_FREIGHT(p_array_size_start,start_date,end_date,sCust_start,sFilterBy);
     If F_IS_TABLE_EEMPTY('TMP_ALL_FREIGHT_ALL') <= 0 Then
        ...
        --  || ' Need to RUN_ONCE F_EOM_TMP_ALL_FREIGHT_ALL 1 for all customers as table is empty.'
       ...
       -- IQ_EOM_PROD.F_EOM_TMP_VAN_FREIGHT_ALL(p_array_size_start,start_date,end_date,sCust_start);
        IQ_EOM_PROD.F_EOM_TMP_ALL_FREIGHT_ALL(p_array_size_start,start_date,end_date);
        IQ_EOM_PROD.F8_Z_EOM_RUN_FREIGHT(p_array_size_start,start_date,end_date,sCust_start,sFilterBy);
      ELSIf UPPER(v_query_logfile) != UPPER(v_tmp_date) 
      AND UPPER(v_query_logfile) IS NOT NULL
      OR UPPER(v_query_result2) != UPPER(sCust_start) 
      Then
        --DBMS_OUTPUT.PUT_LINE(''
        --  || '5th this suggests that BOTH the bulk freight data AND the customer freight data is NOT fresh - re run both!!!'
        --  || ' Need to RUN_ONCE F_EOM_TMP_ALL_FREIGHT_ALL 1 for all customers as table is not empty.'
        ---  || ' Last Cust match was ' ||  UPPER(v_query_result2) 
        --  || ' and this cust was ' ||  UPPER(sCust_start)
        --  || ' and to date was ' ||  UPPER(v_query_logfile)
        --  || ' and this date was ' ||  UPPER(v_tmp_date) 
      ...
      --  Then
          ...
          --|| ' No Need to RUN_ONCE F_EOM_TMP_ALL_FREIGHT_ALL 1 for all customers as table is not empty.'
          --|| ' Last Cust match was ' ||  UPPER(v_query_result2) 
          --|| ' and this cust was ' ||  UPPER(sCust_start)
          --|| ' and to date was ' ||  UPPER(v_query_logfile)
          --|| ' and this date was ' ||  UPPER(v_tmp_date) 
        ...
        Then
         ...
         -- || ' Need to RUN_ONCE F_EOM_TMP_ALL_FREIGHT_ALL 1 for all customers as table is from different dates and different customer.' 
        ...
        AND UPPER(v_query_result2) != UPPER(sCust_start) 
        Then
          --this suggests that the bulk storage data is fresh BUT the ustomer storage data is NOT fresh - re run just cust data
          --DBMS_OUTPUT.PUT_LINE(''
        ...
        --DBMS_OUTPUT.PUT_LINE(''
         ...
         -- IQ_EOM_PROD.F_EOM_TMP_VAN_FREIGHT_ALL(p_array_size_start,start_date,end_date,sCust_start);
          ...
          IQ_EOM_PROD.F8_Z_EOM_RUN_FREIGHT(p_array_size_start,start_date,end_date,sCust_start,sFilterBy);      
        Else
          DBMS_OUTPUT.PUT_LINE('5th No matches for running F_EOM_TMP_ALL_FREIGHT_ALL 1,' 
          || 'still took ' || (round((dbms_utility.get_time-l_start)/100, 6)) || 'Seconds...'
          || ' for customer ' || sCust_start);
        End If;
     
      nCheckpoint := 6;
        Select (F_EOM_CHECK_LOG(v_tmp_date ,'TMP_STOR_ALL_FEES','H4_EOM_ALL_STOR_FEES')) INTO v_query_logfile From Dual;--v_query := q'{Select IQ_EOM_PROD.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'TMP_ALL_FREIGHT_ALL','F_EOM_TMP_ALL_FREIGHT_ALL') }';--q'{INSERT INTO TMP_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_TMP_ALL_FREIGHT_ALL','NONE','TMP_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
        Select (F_EOM_CHECK_CUST_LOG(sCust_start ,'TMP_STOR_FEES','H4_EOM_ALL_STORS')) INTO v_query_result2 From Dual;--v_query := q'{Select IQ_EOM_PROD.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'TMP_ALL_FREIGHT_ALL','F_EOM_TMP_ALL_FREIGHT_ALL') }';--q'{INSERT INTO TMP_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_TMP_ALL_FREIGHT_ALL','NONE','TMP_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
        If F_IS_TABLE_EEMPTY('TMP_STOR_ALL_FEES') <= 0 Then
          --DBMS_OUTPUT.PUT_LINE(''
         ...
         -- || ' and this date was ' ||  UPPER(v_tmp_date) 
         -- );
          IQ_EOM_PROD.H4_EOM_ALL_STOR_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);
          IQ_EOM_PROD.H4_EOM_ALL_STOR(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
        ELSIf UPPER(v_query_logfile) != UPPER(v_tmp_date) 
        AND UPPER(v_query_logfile) IS NOT NULL
        AND UPPER(v_query_result2) != UPPER(sCust_start) 
        Then
      ...
      IQ_EOM_PROD.E0_ALL_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      
    /*IQ_EOM_PROD.E1_PHONE_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);
      nCheckpoint := 72;
      IQ_EOM_PROD.E2_EMAIL_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);
      nCheckpoint := 73;
      IQ_EOM_PROD.E3_FAX_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);
     */  
      nCheckpoint := 74;
      IQ_EOM_PROD.E4_STD_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
     
      nCheckpoint := 75;
      IQ_EOM_PROD.E5_DESTOY_ORD_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);

      nCheckpoint := 81;
      EOM_REPORT_PKG_TEST.G1_SHRINKWRAP_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start);--,sFilterBy);
      nCheckpoint := 82;
      IQ_EOM_PROD.G2_STOCK_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      --nCheckpoint := 83;
      --IQ_EOM_PROD.G3_PACKING_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      ...
      If F_IS_TABLE_EEMPTY('TMP_HANDLING_FEES') <= 0 Then
        ...
        --|| ' and this date was ' ||  UPPER(v_tmp_date) 
       -- );
        IQ_EOM_PROD.G4_HANDLING_FEES_F(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      ELSIf UPPER(v_query_result2) != UPPER(sCust_start) 
      AND UPPER(v_query_logfile) IS NOT NULL 
      ...
      nCheckpoint := 85;
      Select (F_EOM_CHECK_CUST_LOG(sCust_start ,'TMP_PICK_FEES','G5_PICK_FEES_F')) INTO v_query_result2 From Dual;--v_query := q'{Select IQ_EOM_PROD.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'TMP_ALL_FREIGHT_ALL','F_EOM_TMP_ALL_FREIGHT_ALL') }';--q'{INSERT INTO TMP_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_TMP_ALL_FREIGHT_ALL','NONE','TMP_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
      ...
      If F_IS_TABLE_EEMPTY('TMP_PICK_FEES') <= 0 Then
       ...
       -- || ' and this date was ' ||  UPPER(v_tmp_date)
       --  );
        IQ_EOM_PROD.G5_PICK_FEES_F(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      ELSIf UPPER(v_query_result2) != UPPER(sCust_start) 
      AND UPPER(v_query_result2) IS NOT NULL 
      AND UPPER(v_query_logfile) != UPPER(v_tmp_date) Then
      ...
      END IF;
      nCheckpoint := 9;
      IQ_EOM_PROD.I_EOM_MISC_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);

      nCheckpoint := 10;
      IQ_EOM_PROD.K1_PAL_DESP_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      nCheckpoint := 11;
      IQ_EOM_PROD.K2_CTN_IN_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      nCheckpoint := 12;
      IQ_EOM_PROD.K3_PAL_IN_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      nCheckpoint := 13;
      IQ_EOM_PROD.K4_CTN_DESP_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);


     If ( sCust_start = 'VHAAUS' ) Then
        nCheckpoint := 14;
        IQ_EOM_PROD.J_EOM_CUSTOMER_FEES_VHA(p_array_size_start,start_date,end_date,sCust_start);
      ElsIf ( sCust_start = 'BEYONDBLUE' ) Then
        nCheckpoint := 15;
        IQ_EOM_PROD.J_EOM_CUSTOMER_FEES_BB(p_array_size_start,start_date,end_date,sCust_start);
      ElsIf ( sCust_start = 'TABCORP' ) Then
        nCheckpoint := 16;
        IQ_EOM_PROD.J_EOM_CUSTOMER_FEES_TAB(p_array_size_start,start_date,end_date,sCust_start);
      --ElsIf ( sCust_start = 'IAG' ) Then
        --nCheckpoint := 60;
        --IQ_EOM_PROD.Z_EOM_RUN_IAG(p_array_size_start,start_date,end_date,'CGU',sAnalysis_Start);
      End If;
    ...
    --' Seconds...' );
      v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
      IQ_EOM_PROD.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Z_EOM_RUN_ALL','MERGE','NULL',v_time_taken,SYSTIMESTAMP,sCust_start);
  ...
  END Z3_EOM_RUN_ALL;
   ...
   begin
      ...
      -- v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
       --IQ_EOM_PROD.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Z1_TMP_ALL_FEES_TO_CSV','CSV','TMP_ALL_FEES_F',v_time_taken,SYSTIMESTAMP,sCustomerCode);
   ...
   begin
       ...
       execute immediate 'alter session set nls_date_format=''dd-MON-yy'' ';
       --v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
       --IQ_EOM_PROD.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Z2_TMP_FEES_TO_CSV','CSV',p_in_table,v_time_taken,SYSTIMESTAMP,sCustomerCode);
  ...
  BEGIN
     ...
     -- --DBMS_OUTPUT.PUT_LINE(l_data(i).Customer || ' - ' || l_data(i).Parent || '.' );
      --CLOSE EOM_CUSTS;
       ...
       --   --DBMS_OUTPUT.PUT_LINE(l_data(i).Customer || ' - ' || l_data(i).Parent || '.' );
        --END LOOP;
     FOR rec IN (SELECT RM_CUST FROM RM WHERE RM_XX_EOM_ADMIN = 'ADMIN' AND RM_CUST NOT IN (Select CUST FROM TMP_EOM_LOGS WHERE ORIGIN_PROCESS = 'Z_EOM_RUN_ALL' AND SubStr(LAST_SUC_FIN,0,9) = check_date )Order by RM_CUST Desc)
