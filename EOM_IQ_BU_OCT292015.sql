create or replace PACKAGE BODY IQ_EOM_REPORTING AS
    ...
    PROCEDURE G2_STOCK_FEES (
      ...
      SELECT 
          ...
          substr(To_Char(s.SH_ADD_DATE),0,10) AS "OrderDate",
            ...
            ELSE NULL
            END          AS "DIncl",
            CASE WHEN sCustomerCode = 'TABCORP' AND (To_Number(i.IM_REPORTING_PRICE) = 0 OR To_Number(i.IM_REPORTING_PRICE) IS NULL) Then eom_report_pkg.F_BREAK_UNIT_PRICE(r.OW_CAT,d.SD_STOCK)
                 WHEN sCustomerCode = 'TABCORP' AND (To_Number(i.IM_REPORTING_PRICE) != 0 AND To_Number(i.IM_REPORTING_PRICE) IS NOT NULL) Then To_Number(i.IM_REPORTING_PRICE)
  ...
  BEGIN
   v_query := q'{INSERT INTO TMP_ALL_FEES_F
                ...
                WHERE PARENT = :sCust_start}';   
  nCheckpoint := 2;
      EXECUTE IMMEDIATE v_query USING sCustomerCode;
  ...
  BEGIN
   ...
   -- Select (F_EOM_CHECK_LOG(v_tmp_date ,'Tmp_Locn_Cnt_By_Cust','C_EOM_START_ALL_TEMP_STOR_DATA')) INTO v_query_logfile From Dual;--v_query := q'{Select EOM_REPORT_PKG_TEST.EOM_CHECK_LOG(TO_CHAR(end_date,'DD-MON-YY') ,'TMP_ALL_FREIGHT_ALL','F_EOM_TMP_ALL_FREIGHT_ALL') }';--q'{INSERT INTO TMP_EOM_LOGS VALUES (SYSTIMESTAMP ,:startdate,:enddate,'F_EOM_TMP_ALL_FREIGHT_ALL','NONE','TMP_ALL_FREIGHT_ALL',:v_time_taken,SYSTIMESTAMP )  }';
     ...
     */ 
      ...
      nCheckpoint := 81;
      IQ_EOM_REPORTING.G1_SHRINKWRAP_FEES(p_array_size_start,start_date,end_date,sCust_start,sAnalysis_Start,sFilterBy);
      nCheckpoint := 82;
  ...
  END Z3_EOM_RUN_ALL;
    ...
    BEGIN
     ...
     VALUES (v_in_DATETIME,v_in_FROM_DATE,v_in_TO_DATE,v_in_ORIGIN_PROCESS,v_in_ORIGIN_TBL,v_in_DEST_TBL,v_in_TIME_TAKEN,v_in_LAST_TOUCH,v_in_CUST,v_in_LAST_TOUCH );
  ...
  BEGIN
     ...
     -- DBMS_OUTPUT.PUT_LINE(l_data(i).Customer || ' - ' || l_data(i).Parent || '.' );
      --CLOSE EOM_CUSTS;
       ...
       --   DBMS_OUTPUT.PUT_LINE(l_data(i).Customer || ' - ' || l_data(i).Parent || '.' );
        --END LOOP;
     FOR rec IN (SELECT RM_CUST FROM RM WHERE RM_XX_EOM_ADMIN = 'ADMIN' AND RM_CUST IN ('MDA','CABS','HOMTIM','VHAAUS'))
     LOOP
        i := i + 1;
