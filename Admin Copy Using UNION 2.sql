...
AND       To_Number(regexp_substr(r2.RM_XX_FEE03,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) > 0
GROUP BY  s.SH_CUST,
          s.SH_ADD_DATE,
          s.SH_SPARE_STR_4,
          s.SH_ORDER,
...
AND       To_Number(regexp_substr(r2.RM_XX_FEE02,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) > 0 --AND rm3.RM_CUST = :cust
...
--AND       (Select rm3.RM_XX_FEE07 from RM rm3 where rm3.RM_CUST = :cust) <> '0'
--AND       (Select rm3.RM_XX_FEE07 from RM rm3 where rm3.RM_CUST = :cust) IS NOT NULL
AND       To_Number(regexp_substr(r2.RM_XX_FEE07,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) > 0
GROUP BY  s.SH_CUST,
          s.SH_ADD_DATE,
          s.SH_SPARE_STR_4,
          s.SH_ORDER,
...
--AND       (Select rm3.RM_XX_FEE07 from RM rm3 where rm3.RM_CUST = :cust) <> '0'
--AND       (Select rm3.RM_XX_FEE07 from RM rm3 where rm3.RM_CUST = :cust) IS NOT NULL
AND       To_Number(regexp_substr(r2.RM_XX_FEE01,'^[-]?[[:digit:]]*\.?[[:digit:]]*$')) > 0
GROUP BY  s.SH_CUST,
          s.SH_ADD_DATE,
          s.SH_SPARE_STR_4,
          s.SH_ORDER,
...
FROM      PWIN175.SD d
          ...
          INNER JOIN PWIN175.RM r  ON r.RM_CUST  = s.SH_CUST
          INNER JOIN PWIN175.IM i  ON i.IM_STOCK = d.SD_STOCK
WHERE     (Select rmP.RM_XX_FEE25
           from RM rmP
           where To_Number(regexp_substr(rmP.RM_XX_FEE25, '^[-]?[[:digit:]]*\.?[[:digit:]]*$')) > 0 AND rmp.RM_CUST = :cust)  > 0
...
SELECT    s.SH_CUST                AS "Customer",
  ...
  CASE    WHEN d.SD_LINE IS NOT NULL THEN  'Pallet Fee'
          ELSE NULL
          END                     AS "Description",
  CASE    WHEN t.ST_XX_NUM_PALLETS >= 1 THEN  To_Char(t.ST_XX_NUM_PALLETS)
          ELSE NULL
          END                     AS "Qty",
  CASE    WHEN d.SD_LINE IS NOT NULL THEN  '1'
          ELSE NULL
          END                     AS "UOI",
  CASE   WHEN t.ST_XX_NUM_PALLETS >= 1 THEN '' ||  (Select RM_XX_FEE17 from RM where RM_CUST = :cust)
         ELSE null
         END                      AS "UnitPrice",
  CASE   WHEN t.ST_XX_NUM_PALLETS >= 1  THEN '' || (Select RM_XX_FEE17 from RM where RM_CUST = :cust)
         ELSE NULL
  ...
  CASE   WHEN t.ST_XX_NUM_PALLETS >= 1 THEN '' ||  (Select RM_XX_FEE17 from RM where RM_CUST = :cust)
         ELSE ''
         END                                            AS "Excl_Total",
  CASE   WHEN t.ST_XX_NUM_PALLETS >= 1 THEN '' ||  (Select RM_XX_FEE17 from RM where RM_CUST = :cust)  * 1.1
         ELSE ''
         END                                           AS "DIncl",
  CASE   WHEN t.ST_XX_NUM_PALLETS >= 1 THEN '' ||  (Select RM_XX_FEE17 from RM where RM_CUST = :cust)  * 1.1
         ELSE ''
...
WHERE     (Select rmP.RM_XX_FEE17
           from RM rmP
           where To_Number(regexp_substr(rmP.RM_XX_FEE17, '^[-]?[[:digit:]]*\.?[[:digit:]]*$')) > 0 AND rmp.RM_CUST = :cust)  > 0
AND       s.SH_STATUS <> 3
AND       r.RM_ANAL = :anal
AND       s.SH_ORDER = t.ST_ORDER
...
SELECT    s.SH_CUST                AS "Customer",
  ...
  CASE    WHEN d.SD_LINE IS NOT NULL THEN  'Carton Fee'
          ELSE NULL
          END                     AS "Description",
  CASE    WHEN t.ST_XX_NUM_CARTONS >= 1 THEN  To_Char(t.ST_XX_NUM_CARTONS)
          ELSE NULL
          END                     AS "Qty",
  CASE    WHEN d.SD_LINE IS NOT NULL THEN  '1'
          ELSE NULL
          END                     AS "UOI",
  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1 THEN '' ||  (Select RM_XX_FEE15 from RM where RM_CUST = :cust)
         ELSE null
         END                      AS "UnitPrice",
  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1  THEN '' || (Select RM_XX_FEE15 from RM where RM_CUST = :cust)
         ELSE NULL
  ...
  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1 THEN '' ||  (Select RM_XX_FEE15 from RM where RM_CUST = :cust)
         ELSE ''
         END                                            AS "Excl_Total",
  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1 THEN '' ||  (Select RM_XX_FEE15 from RM where RM_CUST = :cust)  * 1.1
         ELSE ''
         END                                           AS "DIncl",
  CASE   WHEN t.ST_XX_NUM_CARTONS >= 1 THEN '' ||  (Select RM_XX_FEE15 from RM where RM_CUST = :cust)  * 1.1
         ELSE ''
...
WHERE     (Select rmP.RM_XX_FEE15
           from RM rmP
           where To_Number(regexp_substr(rmP.RM_XX_FEE15, '^[-]?[[:digit:]]*\.?[[:digit:]]*$')) > 0 AND rmp.RM_CUST = :cust)  > 0
AND       s.SH_STATUS <> 3
AND       r.RM_ANAL = :anal
AND       s.SH_ORDER = t.ST_ORDER
...
SELECT    s.SH_CUST                AS "Customer",
  ...
  CASE    WHEN d.SD_LINE IS NOT NULL THEN  'ShrinkWraping Fee'
          ELSE NULL
          END                     AS "Description",
  CASE    WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN  To_Char(t.ST_XX_NUM_PAL_SW)
          ELSE NULL
          END                     AS "Qty",
  CASE    WHEN d.SD_LINE IS NOT NULL THEN  '1'
          ELSE NULL
          END                     AS "UOI",
  CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN '' ||  (Select RM_XX_FEE18 from RM where RM_CUST = :cust)
         ELSE null
         END                      AS "UnitPrice",
  CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1  THEN '' || (Select RM_XX_FEE18 from RM where RM_CUST = :cust)
         ELSE NULL
  ...
  CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN '' ||  (Select RM_XX_FEE18 from RM where RM_CUST = :cust)
         ELSE ''
         END                                            AS "Excl_Total",
  CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN '' ||  (Select RM_XX_FEE18 from RM where RM_CUST = :cust)  * 1.1
         ELSE ''
         END                                           AS "DIncl",
  CASE   WHEN t.ST_XX_NUM_PAL_SW >= 1 THEN '' ||  (Select RM_XX_FEE18 from RM where RM_CUST = :cust)  * 1.1
         ELSE ''
...
WHERE     (Select rmP.RM_XX_FEE18
           from RM rmP
            where To_Number(regexp_substr(rmP.RM_XX_FEE18, '^[-]?[[:digit:]]*\.?[[:digit:]]*$')) > 0 AND rmp.RM_CUST = :cust)  > 0
AND       s.SH_STATUS <> 3
AND       r.RM_ANAL = :anal
AND       s.SH_ORDER = t.ST_ORDER
