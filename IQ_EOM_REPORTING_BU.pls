...
--   			  3 x Cartons @ 2.43         
    ...
    AS
     ...
     If (sOp = 'PRJ' or sOp = 'PRJ_TEST') Then
         v_query := 'TRUNCATE TABLE DEV_DESP_REPT';
          EXECUTE IMMEDIATE v_query;
        OPEN c;
          DBMS_OUTPUT.PUT_LINE('DEV_DESP_REPT');
          LOOP
          FETCH c BULK COLLECT INTO l_data LIMIT p_array_size;
  ...
  nCheckpoint := 2;
   ...
   begin
       ...
       execute immediate 'alter session set nls_date_format=''dd-MON-yy'' ';
       v_time_taken := TO_CHAR(TO_NUMBER((round((dbms_utility.get_time-l_start)/100, 6))));
       IQ_EOM_REPORTING.EOM_INSERT_LOG(SYSTIMESTAMP ,sysdate,sysdate,'Z1_TMP_ALL_FEES_TO_CSV','CSV','TMP_ALL_FEES_F',v_time_taken,SYSTIMESTAMP,'');
